...
GERMAN_LABEL_REPLACEMENTS = {
    ...
    "## Off and Safe examples": "## Off- und Safe-Beispiele",
    "## Phases": "## Phasen",
...
def _render_configuration(connector: str, data: dict, name: str, partner: str, german: bool) -> list[str]:
    ...
    if german:
        lines += ["## Konfigurationsmodell", "", localized(data, "config", True), "", "## Off, Safe und Strict", "", f"Die [annotierten {name}-Beispiele](../../../examples/{connector}/README.de.md) sind die vollständige lokale Quelle. Ersetzen Sie jeden Pfad, Port und Endpoint für den Zielhost. Off bewahrt die native Interventionsbehandlung ohne zusätzliche Late-Policy; Safe gehört zum dokumentierten Kern; Strict wird nur dort beschrieben, wo der Host und aktuelle Evidence es tragen. Kopieren Sie keine Kompatibilitätskonfiguration als ausgewählten Kernpfad.", "", "## Defaults, Body-Scope und Logging", "", "Nur vorhandene Hostdirektiven und Harness-Optionen sind in den Beispielen aufgeführt. Body- und Content-Type-Verhalten bleiben host- und profilgebunden; keine Konfiguration erlaubt einen connector-eigenen vollständigen Response-Buffer. Nutzen Sie payloadfreie Connector-/Evidence-Logs und speichern Sie keine Secrets in Regeln, Pfaden oder Kommandozeilen.", "", "## Validierung", "", f"Führen Sie `make check-config-{connector}` vor dem Start aus. Der Kernlauf `make full-lifecycle-{connector}` benötigt beschreibbare Runtime- und Evidence-Roots; prüfen Sie das Ergebnis immer anhand der Artefakte.", ""]
    else:
        lines += ["## Configuration model", "", data["config"], "", "## Off, Safe, and Strict", "", f"The [annotated {name} examples](../../../examples/{connector}/README.md) are the complete local source. Replace every path, port, and endpoint for the target host. Off preserves native intervention handling without the additional late policy; Safe belongs to the documented core; Strict is described only where the host and current evidence support it. Do not copy a compatibility configuration as the selected core path.", "", "## Defaults, body scope, and logging", "", "Only existing host directives and harness options are listed in the examples. Body and content-type behavior remain host- and profile-bound; no configuration permits a connector-owned full response buffer. Use payload-free connector/evidence logs and keep secrets out of rules, paths, and command lines.", "", "## Validation", "", f"Run `make check-config-{connector}` before starting a host. The core run `make full-lifecycle-{connector}` needs writable runtime and evidence roots; always inspect its artifacts for the result.", ""]
    lines += variables_table(data, german)
    return lines
