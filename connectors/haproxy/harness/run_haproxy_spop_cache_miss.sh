...
try:
    ...
    agent = subprocess.Popen([
        # This raw-SPOP control deliberately keeps P3/P4 adapter processing
        # disabled.  The #344 baseline requires the native-HTX bridge for
        # response phases; this regression exercises its legacy correlation
        # cache contract directly.
    ...
    if miss["fields"].get("status") != 502 or miss["fields"].get("error") != "stateful_response_transaction_missing_closed":
