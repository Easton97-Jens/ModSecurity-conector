...
| Diagnostic HAProxy-to-agent runtime | diagnostic-enforcement-verified | `make smoke-haproxy` records fresh NOTIFY, request arg extraction, live ModSecurity 403, set-var ACK, block 403, and pass 200 |
| ModSecurity binding | live-enforcement-verified | local libmodsecurity C API signatures verified; HAProxy enforces the phase-1 header block decision over SPOA |
...
| Harness | PASS for one runtime-smoke case | `connectors/haproxy/harness/run_haproxy_smoke.sh` sets `runtime_verified: true` only for `haproxy_phase1_header_block` |
...
| Promotion | partial only | one runtime-smoke case is verified; CRS, RESPONSE_BODY, broader pass-through, and audit/log scopes remain open |
