...
Das begrenzte `MRC1`-Protokoll verwendet Version 2, 12-Byte-Header, maximal
64 KiB pro Frame und maximal 32 KiB je Response-Chunk. Sein ein Byte großer
`CANCEL`-Payload transportiert die kanonische terminale Ursache (Client-Cancel,
Upstream-Disconnect, Connector- oder Protokollfehler, Engine-Timeout/
Unavailable oder ungültige Engineantwort). Es gibt keinen Version-1-Fallback:
ein nicht passender Listener schlägt fail-closed fehl. Das Plugin überträgt
