...
# Reserve a short socket pathname in the already validated private parent.
# The configured task paths can themselves be near the Unix-domain limit.
SOCKET_PATH=$(mktemp "$SOCKET_PARENT"/XXXXXX) || {
    echo "BLOCKED: unable to reserve private engine-service socket path" >&2
    exit 77
}
case "$SOCKET_PATH" in
    "$SOCKET_PARENT"/??????) ;;
    *)
        echo "BLOCKED: unexpected private engine-service socket path: $SOCKET_PATH" >&2
        exit 77
        ;;
esac
[ ! -L "$SOCKET_PATH" ] || {
    echo "BLOCKED: private engine-service socket path is a symlink" >&2
    exit 77
}
rm -- "$SOCKET_PATH"
case "$SOCKET_PATH" in
...
[ "${#SOCKET_PATH}" -le 107 ] || {
    echo "BLOCKED: private engine-service socket path exceeds Unix-domain limit" >&2
    exit 77
}
...
# separate service instance so the bounded first-pass connection count above
# remains deterministic.  The client receive buffers are deliberately small;
# each peer sends valid frames but never reads the RESULT frames, forcing the
# service send path to reach its monotonic 30-second deadline.  All 64 worker
# slots are exercised so the follow-up proves that at least one timed-out
# worker was released before the next request was admitted.
(
    cd "$REPO_ROOT"
    "$ENGINE_BIN" --serve --config "$CONFIG_PATH" --socket "$SOCKET_PATH"
) &
