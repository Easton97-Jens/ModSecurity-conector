...
static int headers_to_common(const envoy_modsecurity_header *src, size_t count, msconnector_header **out, size_t *out_count, char *error, size_t error_len) {
...
void envoy_modsecurity_config_init(msconnector_config *config) { msconnector_config_init(config); msconnector_config_apply_defaults(config); }

static void mapped_request_init(envoy_modsecurity_mapped_request *mapped) { if (mapped) { msconnector_request_init(&mapped->request); mapped->owned_headers = 0; } }
static void mapped_response_init(envoy_modsecurity_mapped_response *mapped) { if (mapped) { msconnector_response_init(&mapped->response); mapped->owned_headers = 0; } }
void envoy_modsecurity_mapped_request_cleanup(envoy_modsecurity_mapped_request *mapped) { if (mapped) { free(mapped->owned_headers); mapped->owned_headers = 0; mapped->request.headers = 0; mapped->request.header_count = 0U; } }
void envoy_modsecurity_mapped_response_cleanup(envoy_modsecurity_mapped_response *mapped) { if (mapped) { free(mapped->owned_headers); mapped->owned_headers = 0; mapped->response.headers = 0; mapped->response.header_count = 0U; } }
...
int envoy_modsecurity_map_owned_request(const envoy_modsecurity_request *src, const msconnector_request_mapper_contract *contract, envoy_modsecurity_mapped_request *out, char *error, size_t error_len) {
    ...
    if (rc != 1) { envoy_modsecurity_mapped_request_cleanup(out); return 0; }
    return 1;
}
...
int envoy_modsecurity_map_request(const envoy_modsecurity_request *src, const msconnector_request_mapper_contract *contract, msconnector_request *out, char *error, size_t error_len) {
    envoy_modsecurity_mapped_request mapped;
    if (out == 0) { mapper_error(error, error_len, "missing request output"); return 0; }
    if (!envoy_modsecurity_map_owned_request(src, contract, &mapped, error, error_len)) { return 0; }
...
int envoy_modsecurity_map_owned_response(const envoy_modsecurity_response *src, const msconnector_response_mapper_contract *contract, envoy_modsecurity_mapped_response *out, char *error, size_t error_len) {
    ...
    if (rc != 1) { envoy_modsecurity_mapped_response_cleanup(out); return 0; }
    return 1;
}
...
int envoy_modsecurity_map_response(const envoy_modsecurity_response *src, const msconnector_response_mapper_contract *contract, msconnector_response *out, char *error, size_t error_len) {
    envoy_modsecurity_mapped_response mapped;
    if (out == 0) { mapper_error(error, error_len, "missing response output"); return 0; }
    if (!envoy_modsecurity_map_owned_response(src, contract, &mapped, error, error_len)) { return 0; }
