...
static int safe_socket_path(const char *path)
{
    ...
    return length > 0U && length < MSCONNECTOR_ENVOY_EXT_AUTHZ_COMPANION_SOCKET_SIZE &&
        strstr(path, "/../") == NULL &&
...
{
    return msconnector_profile_registry_find("envoy-ext-authz");
}
    ...
    uint64_t ttl_ms)
{
    const char *selected = socket_path == NULL || socket_path[0] == '\0' ?
        MSCONNECTOR_ENVOY_EXT_AUTHZ_COMPANION_SOCKET : socket_path;
