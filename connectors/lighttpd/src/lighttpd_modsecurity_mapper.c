...
static int headers_to_common(const lighttpd_modsecurity_header *src, size_t count, msconnector_header **out, size_t *out_count, char *error, size_t error_len) {
...
void lighttpd_modsecurity_config_init(msconnector_config *config) { msconnector_config_init(config); msconnector_config_apply_defaults(config); }

static void mapped_request_init(lighttpd_modsecurity_mapped_request *mapped) { if (mapped) { msconnector_request_init(&mapped->request); mapped->owned_headers = 0; } }
static void mapped_response_init(lighttpd_modsecurity_mapped_response *mapped) { if (mapped) { msconnector_response_init(&mapped->response); mapped->owned_headers = 0; } }
void lighttpd_modsecurity_mapped_request_cleanup(lighttpd_modsecurity_mapped_request *mapped) { if (mapped) { free(mapped->owned_headers); mapped->owned_headers = 0; mapped->request.headers = 0; mapped->request.header_count = 0U; } }
void lighttpd_modsecurity_mapped_response_cleanup(lighttpd_modsecurity_mapped_response *mapped) { if (mapped) { free(mapped->owned_headers); mapped->owned_headers = 0; mapped->response.headers = 0; mapped->response.header_count = 0U; } }
...
int lighttpd_modsecurity_map_owned_request(const lighttpd_modsecurity_request *src, const msconnector_request_mapper_contract *contract, lighttpd_modsecurity_mapped_request *out, char *error, size_t error_len) {
    ...
    if (rc != 1) { lighttpd_modsecurity_mapped_request_cleanup(out); return 0; }
    return 1;
}
...
int lighttpd_modsecurity_map_request(const lighttpd_modsecurity_request *src, const msconnector_request_mapper_contract *contract, msconnector_request *out, char *error, size_t error_len) {
    lighttpd_modsecurity_mapped_request mapped;
    if (out == 0) { mapper_error(error, error_len, "missing request output"); return 0; }
    if (!lighttpd_modsecurity_map_owned_request(src, contract, &mapped, error, error_len)) { return 0; }
...
int lighttpd_modsecurity_map_owned_response(const lighttpd_modsecurity_response *src, const msconnector_response_mapper_contract *contract, lighttpd_modsecurity_mapped_response *out, char *error, size_t error_len) {
    ...
    if (rc != 1) { lighttpd_modsecurity_mapped_response_cleanup(out); return 0; }
    return 1;
}
...
int lighttpd_modsecurity_map_response(const lighttpd_modsecurity_response *src, const msconnector_response_mapper_contract *contract, msconnector_response *out, char *error, size_t error_len) {
    lighttpd_modsecurity_mapped_response mapped;
    if (out == 0) { mapper_error(error, error_len, "missing response output"); return 0; }
    if (!lighttpd_modsecurity_map_owned_response(src, contract, &mapped, error, error_len)) { return 0; }
