...
| Common design note | Current design notes describe selected product routes. | `common/docs/design.md` is marked `scaffolded` and retains historic sidecar/open-connector material. | It conflicts with current route documentation if read as current architecture. | Reconcile or archive historical sections in a separate bilingual documentation task. | Medium | Parent |
