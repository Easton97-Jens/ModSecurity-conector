...
The retained `connectors/nginx/upstream/` tree now contains only `LICENSE`,
`AUTHORS`, `CHANGES`, and `README.md`.
...
materializes `$BUILD_ROOT/nginx-build/connector-src` by combining:

- retained upstream attribution/reference files;
- adapter-owned NGINX `config` and source files;
- generated manifests.
