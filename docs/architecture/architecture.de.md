...
1. Der Connector-Hook empfängt den server/proxy-Requestsstatus.
2. Der Connector-Adapter übersetzt den server/proxy-Status in eine neutrale Requestsansicht.
...
5. Connectorspezifische Tests Evidencen das Hook-Timing und die Artefaktsammlung.

## Gemeinsame Grenze

`common/` enthält nur neutrale Datenformen. Es besitzt kein server/proxy
Objekte und enthält keine Connector-SDK-Header.

`connectors/<name>/` besitzt die gesamte server/proxy-Integration:

- Hakenregistrierung
- module/filter/plugin Baukleber
- Runtimekonfiguration
- server/proxy-specific-Anfrage- und Antwortübersetzung
- connectorspezifische Tests
...
Der Connector darf nur Phasen aufrufen, die er tatsächlich unterstützen und fehlende dokumentieren kann
...
- `blocked`: Die Arbeit kann ohne externe Entscheidung, Quelle oder Prüfung nicht fortgesetzt werden.
