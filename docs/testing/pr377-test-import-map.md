...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `tests/modsecurity-phase4-modes.t` | Minimal mode logs late phase-4 intervention without changing client response | `minimal` | `GET /m` | `Hello minimal` | HTTP 200 body preserved; phase4 log has `actual_action=log_only`, `reason=mode_minimal`, and no response body data | no | `tests/nginx/cases/imported/nginx_phase4_minimal_log_only.yaml` | imported, runtime-blocked in latest NGINX run | Latest NGINX source-built run returned HTTP 403 because generated `htdocs/index.html` was forbidden (permission denied), and `phase4.log` was missing/empty; rerun with an NGINX-readable `BUILD_ROOT` before connector classification | response-body, phase4, logging, pass-through | Not a `RESPONSE_BODY` blocking promotion |
| `tests/modsecurity-phase4-modes.t` | Safe mode logs late phase-4 intervention without changing client response | `safe` | `GET /s` | `Hello safe` | HTTP 200 body preserved; phase4 log has `actual_action=log_only`, `reason=mode_safe`, and no response body data | no | `tests/nginx/cases/imported/nginx_phase4_safe_log_only.yaml` | imported, runtime-blocked in latest NGINX run | Latest NGINX source-built run returned HTTP 403 because generated `htdocs/index.html` was forbidden (permission denied), and `phase4.log` was missing/empty; rerun with an NGINX-readable `BUILD_ROOT` before connector classification | response-body, phase4, logging, pass-through | Not a `RESPONSE_BODY` blocking promotion |
| `tests/modsecurity-phase4-modes.t` | Strict mode aborts after headers were already sent | `strict` | `GET /x` | `Hello strict` | Test::Nginx expects empty reply and phase4 log has `actual_action=connection_abort` | no | `tests/nginx/cases/xfail/nginx_phase4_strict_connection_abort.yaml` | xfail | Current smoke schema asserts stable HTTP status codes; observed curl empty-reply behavior is evidence but not an active PASS | response-body, phase4, intervention, logging | Needs explicit empty-reply/connection-abort assertion before active import |
...
| `tests/modsecurity-phase4-content-types.t` | Out-of-scope content type logs but preserves response | `strict` | `GET /unknown` | `HIT UNKNOWN` with `default_type image/png` | HTTP 200 body preserved; phase4 log has `reason=content_type_not_in_scope` and no response body data | no | `tests/nginx/cases/imported/nginx_phase4_content_type_out_of_scope.yaml` | imported, runtime-blocked in latest NGINX run | Latest NGINX source-built run returned HTTP 403 because generated `htdocs/index.html` was forbidden (permission denied), and `phase4.log` was missing/empty; rerun with an NGINX-readable `BUILD_ROOT` before connector classification | response-body, phase4, logging, pass-through | Does not prove response-body blocking |
...
local NGINX source-built run they are runtime-blocked by generated docroot
permissions, so they currently do not validate connector behavior. They also do
not count as `RESPONSE_BODY` blocking validation because they intentionally
expect HTTP 200.
