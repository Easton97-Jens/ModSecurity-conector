...
| `tests/modsecurity-phase4-content-types.t` | Out-of-scope content type logs but preserves response | `strict` | `GET /unknown` | `HIT UNKNOWN` with `default_type image/png` | HTTP 200 body preserved; phase4 log has `reason=content_type_not_in_scope` and no response body data | no | `tests/nginx/cases/imported/nginx_phase4_content_type_out_of_scope.yaml` | imported | Observed 3/3 PASS on NGINX real connector path; connector-specific NGINX directive behavior | response-body, phase4, logging, pass-through | Does not prove response-body blocking |
...
The three imported NGINX-only PR #377 probes validate phase-4 logging/mode
behavior and pass-through response preservation. They do not count as
`RESPONSE_BODY` blocking validation because they intentionally expect HTTP 200.
