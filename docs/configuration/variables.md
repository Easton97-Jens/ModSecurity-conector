...
| <code>VERIFIED_RUN_PARENT</code> | runtime root | no | <code>RUNNER_TEMP</code>, then <code>TMPDIR</code>, then <code>/var/tmp</code> | absolute directory | Parent of the invocation-owned runtime tree |
...
| Root default | <code>RUNNER_TEMP</code>, then <code>TMPDIR</code>, then <code>/var/tmp</code> | <code>VERIFIED_RUN_ROOT/build</code> | <code>modules/ModSecurity-test-Framework</code> | <code>CACHE_ROOT/shared</code> |
...
| <code>NGINX_HARNESS_PARENT</code>, <code>NGINX_HARNESS_WORK_ROOT</code>, <code>RUNTIME_BASE</code>, <code>RUNTIME_ROOT</code>, <code>LOG_DIR</code>, <code>RESULTS_DIR</code> | Absolute runtime/output directories | Root derives a harness parent; direct harness creates a work child | Controls temporary runtime layout; must be writable and outside <code>/root</code> for worker access |
...
- [Build documentation](../build/README.md) explains target families and safe
  build paths.
- [Testing documentation](../testing/README.md) explains status, selection,
  and validation boundaries.
- [Evidence documentation](../evidence/README.md) explains artifacts and
