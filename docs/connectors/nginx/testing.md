...
Run `make build-nginx`, `make check-config-nginx`, any available start/runtime smoke, and `make full-lifecycle-nginx` as separate levels. Build, configuration, and start are not rule-engine PASS.
