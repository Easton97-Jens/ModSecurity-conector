...
Before response commitment, a host may take a supported pre-commit action. After commitment, Safe records `log_only`; Safe does not claim a rewritten visible status. Host-patch, compression, optional protocol profiles, and strict enforcement remain explicitly bounded by current evidence.
