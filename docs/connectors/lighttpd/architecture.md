...
patched native lighttpd host with `mod_msconnector.so`. Common receives only neutral mapped values; host APIs, allocation, and callback lifetime remain outside Common.
