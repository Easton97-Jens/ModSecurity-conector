...
`make build-apache` prepares the selected host path; `make check-config-apache` performs the configuration check.
...
Run `make build-apache` and then `make check-config-apache`. Start and runtime smokes exist only for the target families that provide them; the selected core run is `make full-lifecycle-apache`. A successful build or config check is not a rule-engine PASS.
