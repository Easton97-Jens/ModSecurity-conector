...
Run `make build-apache`, `make check-config-apache`, any available start/runtime smoke, and `make full-lifecycle-apache` as separate levels. Build, configuration, and start are not rule-engine PASS.
