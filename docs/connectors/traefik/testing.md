...
Run `make build-traefik`, `make check-config-traefik`, any available start/runtime smoke, and `make full-lifecycle-traefik` as separate levels. Build, configuration, and start are not rule-engine PASS.
