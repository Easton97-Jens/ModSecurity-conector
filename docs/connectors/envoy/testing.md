...
Run `make build-envoy`, `make check-config-envoy`, any available start/runtime smoke, and `make full-lifecycle-envoy` as separate levels. Build, configuration, and start are not rule-engine PASS.
