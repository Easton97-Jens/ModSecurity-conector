...
`make build-envoy` und `make check-config-envoy` verwenden den ausgewählten `ext_proc`-Hostpfad.
...
Führen Sie `make build-envoy` und danach `make check-config-envoy` aus. Start- und Runtime-Smokes existieren nur für die jeweils angebotenen Targetfamilien; der ausgewählte Kernlauf ist `make full-lifecycle-envoy`. Erfolgreicher Build oder Config-Check ist kein Rule-Engine-PASS.
