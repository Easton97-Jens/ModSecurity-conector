...
| `connectors/apache/tests/run-regression-tests.pl.in` | 482 | upstream-derived TODO | Use `select()`/`poll()` in upstream harness | cleanup | deferred | P3 | adapter-owned-source | Not used by active smokes |
| `connectors/apache/tests/regression/server_root/conf/httpd.conf.in` | 3 | upstream-derived TODO | Upstream regression template configurability | cleanup | deferred | P3 | adapter-owned-source | Retained as configure template |
