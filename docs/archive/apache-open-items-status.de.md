> Status: Historisch
> Ersetzt durch: [aktueller Apache-Connector-Leitfaden](../connectors/apache/README.de.md)
> Datum: Als historischer Issue-/PR-Snapshot bei der Repository-Reorganisation am 2026-07-12 beibehalten
> Evidenzgrenze: Bewertung eines Source- und Issue-Stands vom 2026-05-24; keine aktuelle kanonische Evidence.

# Apache Connector: Status Offener Punkte

**Sprache:** [English](apache-open-items-status.md) | Deutsch


Status: Historischer Analyse-Snapshot zu Issues/PRs.
...
| Issue | 23 | Configuration merge is not working as expected | Teilweise umgesetzt | Directory/Location-Merge, Ruleset-Merge, Enable/Disable | Hoch | `msc_hook_merge_config_directory`; `ci/checks/connectors/apache/check-apache-directive-config.sh` testet `modsecurity off` in `Location` | Merge-Reihenfolge und Directory-vs-Location-Regelvererbung testen |
| Issue | 24 | Module name should be investigated | Nicht umgesetzt | Modulname und LoadModule-Kompatibilität | Niedrig | Modul bleibt `mod_security3.so` und `security3_module` in `connectors/apache/Makefile.am`, `docs/build/compilers/apache.md` | Entscheidung dokumentieren oder Alias-/Installationsstrategie entwerfen |
...
| Issue | 55 | ./configure fails... configure: error: ModSecurity libraries not found! | Teilweise umgesetzt | Build-Dokumentation und libmodsecurity-Erkennung | Mittel | `docs/build/compilers/apache.md` dokumentiert `--with-libmodsecurity`; `find_libmodsec.m4` bleibt prefix-orientiert | `pkg-config`, explizite Lib/Header-Pfade und bessere Fehlermeldungen ergänzen |
...
| Issue | 77 | What does "ModSecurity-apache is unstable" mean, exactly? | Teilweise umgesetzt | Dokumentation und v2-Konfigurationsunterschiede | Mittel | `README.md`, `docs/build/compilers/apache.md`, `docs/connectors/directive-parity.md` dokumentieren aktuellen Stand und Lücken | Benutzerorientierte "v2 to v3 Apache config" Doku ergänzen |
...
| Issue | 83 | modsec3 module not loaded for Linux 7.2 os version | Teilweise umgesetzt | ABI/API-Kompatibilität zu libmodsecurity | Mittel | `msc_new_transaction_with_id` wird verwendet; `docs/build/compilers/apache.md` dokumentiert Load-/ldd-Mismatch | Version-/Symbol-Check oder klare Mindestversion im configure ergänzen |
| Issue | 84 | Unable to disable module once loaded | Umgesetzt | `modsecurity off` in Apache-Kontexten | Hoch | `msc_config_modsec_state`, `create_tx_context`; `ci/checks/connectors/apache/check-apache-directive-config.sh` erwartet `Location`-Bypass mit HTTP `200` | Framework-Test für vhost/location-disable ergänzen |
...
- Empfohlene nächste Schritte: CRS-Fixtures in `ModSecurity-test-Framework` aufnehmen, `ci/runtime/run-apache-smoke.sh` um CRS-Profil ergänzen, Log-Parser-Ausgabe als Artefakt persistieren. Vermuteter Aufwand: hoch.
...
- Relevante Tests im ModSecurity-test-Framework: Keine umfassende Directory-/Location-Merge-Matrix. Connector-eigener Gate: `ci/checks/connectors/apache/check-apache-directive-config.sh`.
...
- Relevante Dateien im ModSecurity-conector: `connectors/apache/Makefile.am`, `connectors/apache/build/apxs-wrapper.in`, `docs/build/compilers/apache.md`.
...
- Relevante Tests im ModSecurity-test-Framework: `ci/checks/connectors/apache/check-apache-directive-config.sh` im Connector; Framework hat nur indirekte Regeltests.
...
- Relevante Tests im ModSecurity-test-Framework: `ci/provisioning/prepare-apache-build.sh` nutzt einen staging prefix, testet aber nicht Arch-Layout oder `DESTDIR`.
...
- Relevante Dateien im ModSecurity-conector: `docs/build/compilers/apache.md`, `connectors/apache/build/find_libmodsec.m4`, `connectors/apache/docs/build.md`.
- Relevante Tests im ModSecurity-test-Framework: `ci/provisioning/prepare-apache-build.sh` mit `./configure --with-libmodsecurity=$MODSECURITY_STAGE`.
...
- Relevante Dateien im ModSecurity-conector: `connectors/apache/src/msc_config.c`, `README.md`, `docs/build/compilers/apache.md`, `docs/connectors/directive-parity.md`.
...
- Empfohlene nächste Schritte: `docs/build/compilers/apache.md` um v2-zu-v3-Konfigurationsbeispiele erweitern; Startup-Fehlertest für direkte v2-Direktiven ergänzen. Vermuteter Aufwand: mittel.
...
- Empfohlene nächste Schritte: In `ci/checks/connectors/apache/check-apache-directive-config.sh` oder Framework einen Error-Log-Assert für Blocking-Regeln aufnehmen. Vermuteter Aufwand: mittel.
...
- Relevante Dateien im ModSecurity-conector: `README.md`, `docs/build/compilers/apache.md`, `docs/connectors/directive-parity.md`, `reports/testing/test-coverage-overview.md`.
...
- Typ: Issue
- Statusbewertung: Nicht relevant
- Kurzbeschreibung: Projektplanungsfrage ohne konkrete technische Request.
...
- Relevante Tests im ModSecurity-test-Framework: Connector-eigener Test `ci/checks/connectors/apache/check-apache-directive-config.sh`.
...
- Empfohlene nächste Schritte: Leak-Repro in `ci/runtime/run-apache-smoke.sh` oder separatem Langtest ergänzen; Ruleset-Cleanup auditieren. Vermuteter Aufwand: hoch.
...
- Relevante Dateien im ModSecurity-conector: `connectors/apache/src/mod_security3.c` nutzt `msc_new_transaction_with_id`; `docs/build/compilers/apache.md` enthält Load-/ldd-Troubleshooting.
...
- Relevante Tests im ModSecurity-test-Framework: Connector-eigener `ci/checks/connectors/apache/check-apache-directive-config.sh` mit `Location "/__modsec_directive_off"`.
