...
- `tests/runners/case_cli.py` reads each YAML file and materializes the Apache
  rules, request method/path, headers, body, and expected HTTP status for the
  harness.
...
the observed Apache response against each YAML expectation. A successful compile
