...
<code>.go-version</code> selector (currently Go <code>1.27.1</code>), and a
...
owns its Go language baseline. At its bounded scheduled resolution, the
updater selects the greatest stable numeric Go release and proposes it in a
Draft PR after read-only candidate validation. It may change only
<code>.go-version</code> and the fixed, independently validated Envoy
component bundle; it cannot alter arbitrary module or dependency files. The
C/C++ result does not claim full connector coverage; expanding it requires
reproducible builds for the selected connector scope.
