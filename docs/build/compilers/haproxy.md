...
HAProxy build details depend on the logical profile. The direct
`haproxy-htx` route uses the patched native HTX integration; the
`haproxy-spoe-spop` solution also includes the request-side agent/companion
topology. Start with the root build targets and use this page for native
toolchain/source requirements. Build success is not full profile evidence.
...
libmodsecurity v3, HAProxy 3.2.23 source, the repository native HTX filter/overlay, the Common bridge, a local rule file, a loopback frontend, and a loopback upstream.
...
- **Source and scope:** [HAProxy Releases](https://www.haproxy.org/download/)
  Official source downloads and release series selection. Version scope: The repository overlay currently fixes its compatible source to 3.2.23.
...
mkdir -p "$WORKDIR"
cd "$WORKDIR"
curl -fLO "https://www.haproxy.org/download/3.2/src/haproxy-3.2.23.tar.gz"
curl -fL "https://www.haproxy.org/download/3.2/src/haproxy-3.2.23.tar.gz.sha256" -o "haproxy-$VERSION.tar.gz.sha256"
...
export HAPROXY_SRC="$HOST_BUILD_BASE/haproxy-3.2.23"
...
The overlay refuses a version other than 3.2.23, an in-tree build directory, missing libmodsecurity headers, or a missing library. Treat that as a compatibility boundary, not as a reason to substitute a SPOA result.
