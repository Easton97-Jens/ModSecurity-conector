...
# Build-, Source- und Paketwege: NGINX
...
`native-nginx-http-module` für NGINX: natives NGINX-HTTP-Modul. Der kanonische Kernlauf ist
`make full-lifecycle-nginx`. Build-, Konfigurations-, Start- und Kompatibilitäts-
...
| `make build-nginx` | Buildstufe | Vorbereitung und Toolchain | `$BUILD_ROOT/stages/nginx/build/results` | `0` ist Stufenerfolg, kein Config- oder Trafficnachweis. |
| `make check-config-nginx` | Konfiguration laden/prüfen | erzeugter Host/Connector | `$BUILD_ROOT/stages/nginx/config_load/results` | `0` ist kein gesendeter HTTP-Request. |
| `make start-smoke-nginx` | Host ohne Volltraffic starten | lesbare Konfiguration und freie lokale Ressourcen | `$BUILD_ROOT/stages/nginx/start_smoke/results` | `0` ist keine Full-Lifecycle-Evidence. |
| `make runtime-smoke-nginx` | begrenzten repository-eigenen Runtime-Smoke ausführen | vorbereiteter Host und lokale Ports | `$BUILD_ROOT/stages/nginx/minimal_runtime_smoke/results` | `0` gilt nur für diesen Smoke. |
| `make full-lifecycle-nginx` | ausgewählten No-CRS-Kernlauf ausführen | sicherer Run-Identifier | `$VERIFIED_RUN_ROOT/evidence/no-crs-evidence/nginx/$run_id` | Kanonische Artefakte erst nach anschließendem Evidence-Check bewerten. |
| `make evidence-check-nginx` | bereits erzeugte kanonische Artefakte validieren | derselbe Run-Identifier und vollständige Artefakte | `$VERIFIED_RUN_ROOT/evidence/no-crs-evidence/nginx/$run_id` | validiert vorhandene Evidence; erzeugt keine neuen Logs oder Runtime-Dateien. |
...
export VERIFIED_RUN_ROOT="$VERIFIED_RUN_PARENT/ModSecurity-conector-verified"
export CACHE_ROOT="$VERIFIED_RUN_ROOT/cache-v2"
export BUILD_ROOT="$VERIFIED_RUN_ROOT/build/nginx-source"
export CC=gcc
export CXX=g++
export CFLAGS="-O2 -g"
...
make runtime-components-inventory
make runtime-components-sources
run_id="nginx-source-$(date -u +%Y%m%dT%H%M%SZ)"
...
| `make full-lifecycle-nginx` + Evidence-Check | ausgewählten Kernpfad ausführen und Artefakte validieren | sicherer `run_id` und vollständige Runtime | Evidence unter `evidence/no-crs-evidence/nginx/$run_id`; `2` steht für ungültige Eingabe/Stufe, andere Fehler bleiben Fehler. |

Der unterstützte Build wird dabei durch `modules/ModSecurity-test-Framework/ci/provisioning/prepare-nginx-build.sh` umgesetzt; die
...
# Debian / Ubuntu (apt)
apt-cache policy build-essential pkg-config git curl ca-certificates
apt-cache policy libpcre2-dev zlib1g-dev libssl-dev libmodsecurity-dev nginx nginx-dev
# Fedora / RHEL / Rocky Linux / AlmaLinux (dnf)
dnf info gcc gcc-c++ make pkgconf-pkg-config git curl ca-certificates
dnf info pcre2-devel zlib-devel openssl-devel libmodsecurity-devel nginx
```
...
sudo apt install --yes libpcre2-dev zlib1g-dev libssl-dev libmodsecurity-dev nginx nginx-dev
# Fedora / RHEL / Rocky Linux / AlmaLinux (dnf)
sudo dnf install -y gcc gcc-c++ make pkgconf-pkg-config git curl ca-certificates
sudo dnf install -y pcre2-devel zlib-devel openssl-devel libmodsecurity-devel nginx
```
...
export VERIFIED_RUN_ROOT="$VERIFIED_RUN_PARENT/ModSecurity-conector-verified"
export CACHE_ROOT="$VERIFIED_RUN_ROOT/cache-v2"
export BUILD_ROOT="$VERIFIED_RUN_ROOT/build/nginx-package"
jobs="$(getconf _NPROCESSORS_ONLN 2>/dev/null || printf '2')"
make check-framework
make prepare-runtime-components
make runtime-components-inventory
make runtime-components-sources
run_id="nginx-package-$(date -u +%Y%m%dT%H%M%SZ)"
BUILD_NGINX_FROM_SOURCE=1 NGINX_SOURCE_MODE=github-release MAKE_JOBS="$jobs" make build-nginx
make check-config-nginx
make start-smoke-nginx
make runtime-smoke-nginx
...
`make runtime-smoke-nginx` ist der unterstützte repository-eigene
...
```sh
run_id="nginx-http11-$(date -u +%Y%m%dT%H%M%SZ)"
NO_CRS_RUN_ID="$run_id" make full-lifecycle-nginx
NO_CRS_RUN_ID="$run_id" make evidence-check-nginx
```
...
`evidence/no-crs-evidence/nginx/$run_id`, Builddateien in
`build/nginx/$run_id`, Runtime-Dateien in `runs/nginx/$run_id` und
sanitisierte Logs in `run-logs/nginx/$run_id`. Allgemeine Stufenresultate
liegen unter `$BUILD_ROOT/stages/nginx`. Pfade sind abgeleitet, nicht feste
Systempfade.
...
```sh
sudo apt remove nginx nginx-dev
...
| BUILD_NGINX_FROM_SOURCE | nein | 1 | 1 | Erfordert den Framework-gesteuerten NGINX-Source-Build. |
| NGINX_SOURCE_MODE | nein | github-release | github-release | Unterstützter NGINX-Quellbezugmodus. |
| NGINX_SOURCE_REPO_URL | nein | Framework default | https://github.com/nginx/nginx | HTTPS-GitHub-Repository für den Release-Bezug. |
| NGINX_SOURCE_GIT_REF | nein | release-1.31.2 | release-1.31.2 | Ausgewählte NGINX-Git-Referenz; die Provenienz ist maßgeblich. |
| NGINX_RELEASE_TAG | nein | release-1.31.2 | release-1.31.2 | Angefragtes NGINX-Release-Tag; die Vorbereitung protokolliert die aufgelöste Release-Provenienz. |
| NGINX_BIN | nein | resolved by preparation | external selected nginx path | NGINX-Executable, das mit dem ausgewählten dynamischen Modul zusammengehört. |
| NGINX_PREFIX | nein | generated external staging prefix | external staging prefix | Host-Prefix, der mit der Modul-ABI zusammenbleiben muss. |
| NGINX_MODULE | nein | generated module path | external module path | Erzeugter nativer Modulpfad der ausgewählten Konfiguration. |

| Dokumentierter Wert | Beispiel | Bedeutung |
| --- | --- | --- |
| Connectorname | nginx | Make- und Evidence-Name dieses Guides; kein Platzhalter in den gezeigten Befehlen. |
...
| Build-Verzeichnis | `$BUILD_ROOT/stages/nginx` | Staging- und Stufenergebnisse außerhalb des Checkouts. |
