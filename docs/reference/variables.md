...
| <code>COMPDB_OUTPUT</code> | local diagnostics | required by compilation-database and clangd targets | none | absolute writable file path outside the checkout | Atomically published Bear compilation database; captures merge by translation unit and never overwrite a failed target |
