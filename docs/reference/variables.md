...
| <code>TRAEFIK_ENGINE_SOCKET_PARENT</code> | Existing private absolute parent directory for the native engine UDS child | Explicit value, then <code>TMPDIR</code>, otherwise a short generated private parent below <code>/var/tmp</code> | Explicit/<code>TMPDIR</code> values must be current-user owned, <code>0700</code>-private, outside the checkout, and free of symlink components; broad roots and control characters are rejected. On Linux the C engine self-probes the pathname with <code>SO_PEERCRED</code> before readiness and compares identity on both sides of that bounded capture, so a replacement in that pre-readiness window fails closed. It does not bind later middleware dials to the captured listener: <code>0700</code> separates UIDs, not hostile processes sharing the same UID. The runner checks generated child/fallback identity and emptiness before removal and never removes a caller-selected parent or socket path, but POSIX has no atomic conditional unlink/rmdir; neither cleanup nor live endpoint identity is same-UID race-proof. |
| <code>TRAEFIK_ENGINE_SOCKET_TEST_PARENT</code> | Private parent for the focused engine-service runtime test only | <code>/var/tmp</code> allocation root | Optional sandbox/test override; a non-default value must be absolute, current-user owned, and <code>0700</code>-private. It does not configure a host probe. |
