...
| <code>NGINX_DOCROOT_PROJECTION</code>, <code>NGINX_DOCROOT_PROJECTION_PARENT</code>, <code>NGINX_DOCROOT_PROJECTION_ROOT</code> | Opt-in worker-visible static docroot projection; boolean plus absolute parent and exact child paths | <code>0</code>; parent/root empty unless supplied by the lifecycle manifest | Projection mode fails closed unless the manifest pre-registers both an external parent and an exact fresh direct child. The parent must already exist outside private runtime roots, be root-owned, symlink-free, group/other-nonwritable and non-readable, and worker-traversable in a <code>0711</code>-safe form with traversable ancestors. The helper validates but never creates, <code>chown</code>s, or <code>chmod</code>s that parent; it creates only the fresh direct child, copies the allowlisted static files, and makes that child worker-traversable. This is not authorization for generic harness output outside <code>VERIFIED_RUN_ROOT</code>. |
