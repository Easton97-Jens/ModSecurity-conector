...
| <code>--evidence-root</code>, <code>--results-dir</code>, <code>--tmp-root</code>, <code>--log-root</code>, <code>--log-dir</code>, <code>--config-root</code> | direct local runtime smoke | yes for <code>run_local_runtime_smoke.py</code> | selected invocation layout | absolute, canonical descendants of <code>VERIFIED_RUN_ROOT</code> | The runner validates every destination before its first create, unlink, generated-config, evaluator, or result-writer path; a relative, outside-root, or symlink-component escape—including the fixed CRS config suffix—is blocked with exit 77 |
...
| <code>FULL_MATRIX_MAX_PARALLEL_JOBS</code>, <code>FULL_MATRIX_PORT_SPAN</code> | full-matrix runtime scheduler | no | detected online CPU count (<code>nproc</code>, then <code>getconf _NPROCESSORS_ONLN</code>, otherwise <code>1</code>); <code>1000</code> | positive decimal integer | Caps globally scheduled isolated runtime jobs and their listener search ranges. The runner first validates every Apache, NGINX, and HAProxy port tuple; missing prepared artifacts retain serial execution. The verified-report workflow invokes the runner's read-only <code>--print-effective-parallelism</code> diagnostic before the strict producer so its dynamically selected cap is visible without pinning a runner-specific value. |
...
| <code>CRS_REPO_URL</code>, <code>CRS_GIT_REF</code>, <code>CRS_SOURCE_DIR</code>, <code>CRS_RUNTIME_DIR</code>, <code>MODSECURITY_RULE_PREAMBLE_FILE</code> | optional CRS input | no | explicit prepared source or explicit runtime-lookup root | URL/ref; absolute trusted directory/path | CRS source plus generated configuration and audit-log directories must be non-symlinked, runner/root-owned, non-group/world-writable, and safe through every included setup, rule, and plugin path; directive paths reject quotes, backslashes, line breaks, and glob metacharacters, and the runner never discovers CRS below shared temporary locations |
| <code>BUILD_HTTPD_FROM_SOURCE</code>, <code>BUILD_PCRE2_FROM_SOURCE</code>, <code>BUILD_NGINX_FROM_SOURCE</code>, <code>NGINX_SOURCE_MODE</code>, <code>NGINX_SOURCE_REPO_URL</code>, <code>NGINX_SOURCE_GIT_REF</code>, <code>NGINX_GITHUB_REPO</code>, <code>NGINX_RELEASE_TAG</code>, <code>NGINX_RELEASE_ASSET_NAME</code>, <code>NGINX_SHA256</code>, <code>NGINX_REQUIRE_PINNED_PROVENANCE</code> | host provisioning | no | Framework-forwarded; full-smoke supplies a fixed tuple | boolean, mode, URL/ref/tag/asset name/SHA-256 | Selects host source acquisition and build policy; strict full-smoke provenance requires the complete fixed NGINX tuple |
| <code>HTTPD_VERSION</code>, <code>HTTPD_SOURCE_URL</code>, <code>HTTPD_SHA256</code>, <code>HTTPD_SHA256_URL</code>, <code>APR_VERSION</code>, <code>APR_SOURCE_URL</code>, <code>APR_SHA256</code>, <code>APR_SHA256_URL</code>, <code>APR_UTIL_VERSION</code>, <code>APR_UTIL_SOURCE_URL</code>, <code>APR_UTIL_SHA256</code>, <code>APR_UTIL_SHA256_URL</code>, <code>PCRE2_VERSION</code>, <code>PCRE2_SOURCE_URL</code>, <code>PCRE2_SHA256</code>, <code>PCRE2_SHA256_URL</code> | Apache provisioning | no | guarded Framework tuple; direct Parent overrides are rejected; Parent owns no APR-util version or SHA-256 and preserves the absent-versus-empty PCRE2 digest boundary | version, URL, SHA-256 | Apache dependency provenance is resolved from the guarded Framework tuple. Only a full canonical APR-util tuple may propagate internally between Parent and Child, and only after an independent comparison with the checked-out Framework; partial, empty, mismatched, or alternate tuples fail closed. Parent neither duplicates nor overrides APR-util provenance. An empty or malformed PCRE2 digest fails closed and <code>PCRE2_SHA256_URL</code> is not an extraction fallback |
...
| <code>EXPAT_SOURCE_URL</code>, <code>EXPAT_GIT_REF</code>, <code>EXPAT_GIT_URL</code>, <code>EXPAT_PROMPT_EXPECTED_LATEST</code> | Apache dependency | no | Framework/provider full commit pin | URL/full immutable Git commit/URL/legacy boolean | Expat runtime resolution always uses <code>EXPAT_GIT_REF</code>, which must be exactly 40 or 64 hexadecimal characters. <code>EXPAT_PROMPT_EXPECTED_LATEST</code> is retained only for backward-compatible input transport and is ignored by Expat runtime resolution. |
...
| <code>NGINX_HARNESS_PARENT</code>, <code>NGINX_HARNESS_WORK_ROOT</code>, <code>RUNTIME_BASE</code>, <code>RUNTIME_ROOT</code>, <code>LOG_DIR</code>, <code>RESULTS_DIR</code> | Absolute runtime/output directories | Root derives a harness parent; direct harness creates a work child | Controls a split temporary layout. Root owns the private runtime/harness material and <code>LOG_DIR</code>, including <code>NGINX_PHASE4_LOG_FILE</code> and the private <code>NGINX_MEMCHECK_EVIDENCE_DIR</code> at <code>LOG_DIR/memcheck-evidence/&lt;case&gt;</code>; only the separate <code>worker-state</code> and <code>server-logs</code> trees below the worker-traversable harness root carry worker-owned leaves. Worker visibility of private output or any overlap fails closed. Before any <code>mkdir</code>, install, <code>chown</code>, <code>chmod</code>, <code>rm</code>, or output redirection, every generic/private bootstrap, parent multi-case, and per-case work/output root—and configurable diagnostic, preflight, protocol, lifecycle, and curl outputs—must validate as strict descendants of <code>VERIFIED_RUN_ROOT</code>; an out-of-root path exits <code>77</code>. <code>/dev/null</code> is allowed only for the internal bounded-soak curl sinks. The separately governed docroot-projection exception is described in the next row. This path-authority control does not change the trusted <code>PYTHON</code>/<code>PATH</code> launch assumption. |
| <code>NGINX_DOCROOT_PROJECTION</code>, <code>NGINX_DOCROOT_PROJECTION_PARENT</code>, <code>NGINX_DOCROOT_PROJECTION_ROOT</code> | Opt-in worker-visible static docroot projection; boolean plus absolute parent and exact child paths | <code>0</code>; parent/root empty unless explicitly supplied by a trusted lifecycle/operator caller | Projection mode fails closed unless the trusted caller supplies both an external parent and an exact fresh direct child. The harness does not look up or enforce lifecycle-manifest registration for those values; it validates instead that the parent already exists outside private runtime roots, is root-owned, symlink-free, group/other-nonwritable and non-readable, and worker-traversable in a <code>0711</code>-safe form with traversable ancestors. The helper never creates, <code>chown</code>s, or <code>chmod</code>s that parent; it creates only the fresh direct child, copies the allowlisted static files, and makes that child worker-traversable. This is not authorization for generic harness output outside <code>VERIFIED_RUN_ROOT</code>. |
| <code>NGINX_WORKER_USER</code>, <code>NGINX_WORKER_GROUP</code>, <code>NGINX_WORKER_PREFLIGHT_FILE</code>, <code>PERMISSIONS_LOG</code> | OS account/group and absolute log file | <code>nobody</code>; group/log derived | Selects the actual NGINX worker identity and its ownership boundary, not merely diagnostics. The direct harness requires root execution and fails closed unless it resolves a distinct verifiable local <code>NGINX_WORKER_USER</code> and group, then renders <code>user &lt;resolved-user&gt; &lt;resolved-group&gt;;</code>. That identity owns only worker state and the server access/error/audit leaves. |
| <code>NGINX_MEMCHECK</code>, <code>NGINX_MEMCHECK_CASES</code>, <code>NGINX_MEMCHECK_DURATION_SECONDS</code>, <code>VALGRIND_BIN</code> | Opt-in H1 Memcheck mode, bounded case selection/duration, and trusted Valgrind executable | <code>NGINX_MEMCHECK=0</code>; <code>make memcheck-nginx</code> supplies its bounded defaults | The mode remains outside default smoke/test/CI. It requires the root-run distinct-worker check and its NGINX binary/archive identity gates. The summarizer trusts only effective-UID-owned, non-symlink, group/other-nonwritable root/parent directories; direct-child private single-link regular evidence files opened no-follow and checked for replacement can contribute to a clean result. A retained pre-hardening artifact is not proof of this current path. The retained remediation receipt <code>$RUN/evidence/direct-nginx-h1-memcheck-evidence-remediation-20260801.md</code> (SHA-256 <code>37f01fe3d1851d43ae21d2b705b02bf01f204ff5cb19b41354e5b801a4b158a8</code>) records a <code>passed_noncanonical_diagnostic</code> under <code>umask 022</code> with a distinct <code>nobody</code> worker and private evidence metadata; it uses a pre-current-C artifact. A separate fresh C build validated prior C code, while final security-scan and final PR-head CI/Sonar evidence remain pending. |
...
| <code>TRAEFIK_ENGINE_SOCKET_PARENT</code> | Existing private absolute parent directory for the native engine UDS child | Required explicit value; absent/invalid input fails closed | The selected parent must be current-user owned, exact-<code>0700</code>, outside the checkout, free of symlink components, and protected against cross-UID replacement through every ancestor. A group- or other-writable ancestor is allowed only when sticky and its next child entry belongs to the effective UID; broad roots and control characters are rejected. On Linux the C engine independently enforces the same contract, then self-probes the pathname with <code>SO_PEERCRED</code> before readiness and compares identity on both sides of that bounded capture, so a replacement in that pre-readiness window fails closed. It does not bind later middleware dials to the captured listener: <code>0700</code> separates UIDs, not hostile processes sharing the same UID. The runner removes only its unchanged empty child and never removes a caller-selected parent or socket path, but POSIX has no atomic conditional unlink/rmdir; neither cleanup nor live endpoint identity is same-UID race-proof. |
| <code>TRAEFIK_ENGINE_SOCKET_TEST_PARENT</code> | Existing private parent for the focused engine-service runtime/self-test only | Required for this focused test | Must be absolute, current-user owned, exact-<code>0700</code>, canonical, non-symlinked, and protected from cross-UID ancestor replacement under the same sticky-directory rule. It has no public-writable default and does not configure a host probe. |
...
The central remaining-connector dispatcher carries
TRAEFIK_ENGINE_SOCKET_PARENT as process-environment data. The native Make
target preserves it with raw GNU Make value transport and export rather than a
recipe shell assignment, so quotes, semicolons, and Make expressions are not
evaluated before the runner validates it. No runtime or temporary root is used
to derive a parent: CI and direct callers must create a sufficiently short
protected parent explicitly, and its absence is a fail-closed BLOCKED
prerequisite.
