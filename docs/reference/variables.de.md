...
| <code>NGINX_DOCROOT_PROJECTION</code>, <code>NGINX_DOCROOT_PROJECTION_PARENT</code>, <code>NGINX_DOCROOT_PROJECTION_ROOT</code> | Opt-in-Worker-sichtbare statische Docroot-Projektion; Boolean sowie absolute Parent- und exakte Kind-Pfade | <code>0</code>; Parent/Root leer, sofern sie nicht vom Lifecycle-Manifest geliefert werden | Der Projection-Modus schlägt fail-closed fehl, sofern das Manifest nicht sowohl einen externen Parent als auch ein exaktes frisches direktes Kind vorregistriert. Der Parent muss außerhalb privater Runtime-Roots bereits existieren, root-eigen, symlink-frei, für Gruppe/Andere nicht beschreibbar und nicht lesbar sowie in einer <code>0711</code>-sicheren Form für den Worker traversierbar sein; auch seine Ancestors müssen traversierbar sein. Der Helper validiert, aber erzeugt, <code>chown</code>t oder <code>chmod</code>t diesen Parent nie; er erzeugt nur das frische direkte Kind, kopiert die allowlisteten statischen Dateien und macht dieses Kind für den Worker traversierbar. Dies autorisiert keinen generischen Harness-Output außerhalb von <code>VERIFIED_RUN_ROOT</code>. |
