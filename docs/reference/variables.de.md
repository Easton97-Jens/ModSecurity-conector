...
| <code>--evidence-root</code>, <code>--results-dir</code>, <code>--tmp-root</code>, <code>--log-root</code>, <code>--log-dir</code>, <code>--config-root</code> | direkter lokaler Runtime-Smoke | ja für <code>run_local_runtime_smoke.py</code> | ausgewähltes Invocation-Layout | absolute, kanonische Nachkommen von <code>VERIFIED_RUN_ROOT</code> | Der Runner prüft jedes Ziel vor dem ersten Erzeugen, Unlink, Generated-Config-, Evaluator- oder Result-Writer-Pfad; ein relativer, außerhalb des Roots liegender oder Symlink-Component-Escape—einschließlich des festen CRS-Config-Suffixes—wird mit Exit 77 blockiert |
...
| <code>FULL_MATRIX_MAX_PARALLEL_JOBS</code>, <code>FULL_MATRIX_PORT_SPAN</code> | Full-Matrix-Runtime-Scheduler | nein | erkannte Online-CPU-Zahl (<code>nproc</code>, dann <code>getconf _NPROCESSORS_ONLN</code>, sonst <code>1</code>); <code>1000</code> | positive dezimale Ganzzahl | Begrenzt global geplante isolierte Runtime-Jobs und ihre Listener-Suchbereiche. Der Runner validiert zuerst jedes Apache-, NGINX- und HAProxy-Porttupel; bei fehlenden vorbereiteten Artefakten bleibt die Ausführung seriell. Der verified-report-Workflow ruft vor dem strikten Producer die schreibgeschützte Runner-Diagnose <code>--print-effective-parallelism</code> auf, damit die dynamisch ausgewählte Obergrenze sichtbar ist, ohne einen runner-spezifischen Wert festzuschreiben. |
...
| <code>HTTPD_VERSION</code>, <code>HTTPD_SOURCE_URL</code>, <code>HTTPD_SHA256</code>, <code>HTTPD_SHA256_URL</code>, <code>APR_VERSION</code>, <code>APR_SOURCE_URL</code>, <code>APR_SHA256</code>, <code>APR_SHA256_URL</code>, <code>APR_UTIL_VERSION</code>, <code>APR_UTIL_SOURCE_URL</code>, <code>APR_UTIL_SHA256</code>, <code>APR_UTIL_SHA256_URL</code>, <code>PCRE2_VERSION</code>, <code>PCRE2_SOURCE_URL</code>, <code>PCRE2_SHA256</code>, <code>PCRE2_SHA256_URL</code> | Apache-Provisionierung | nein | geschütztes Framework-Tupel; direkte Parent-Overrides werden abgewiesen; Parent besitzt keine APR-util-Version oder SHA-256 und erhält die Grenze zwischen fehlendem und leerem PCRE2-Digest | Version, URL, SHA-256 | Die Apache-Dependency-Provenance wird aus dem geschützten Framework-Tupel aufgelöst. Nur ein vollständiges kanonisches APR-util-Tupel darf intern zwischen Parent und Child weitergegeben werden, und nur nach einem unabhängigen Vergleich mit dem ausgecheckten Framework; partielle, leere, abweichende oder alternative Tupel schlagen fehlgeschlossen fehl. Parent dupliziert oder überschreibt die APR-util-Provenance nicht. Ein leerer oder fehlerhafter PCRE2-Digest schlägt fehlgeschlossen fehl und <code>PCRE2_SHA256_URL</code> ist kein Extraction-Fallback |
...
| <code>EXPAT_SOURCE_URL</code>, <code>EXPAT_GIT_REF</code>, <code>EXPAT_GIT_URL</code>, <code>EXPAT_PROMPT_EXPECTED_LATEST</code> | Apache-Dependency | nein | vollständiger Framework-/Provider-Commit-Pin | URL/vollständiger unveränderlicher Git-Commit/URL/Legacy-Boolean | Die Expat-Runtimeauflösung verwendet immer <code>EXPAT_GIT_REF</code>, das exakt 40 oder 64 hexadezimale Zeichen enthalten muss. <code>EXPAT_PROMPT_EXPECTED_LATEST</code> bleibt nur für rückwärtskompatiblen Eingabetransport erhalten und wird von der Expat-Runtimeauflösung ignoriert. |
...
| <code>NGINX_HARNESS_PARENT</code>, <code>NGINX_HARNESS_WORK_ROOT</code>, <code>RUNTIME_BASE</code>, <code>RUNTIME_ROOT</code>, <code>LOG_DIR</code>, <code>RESULTS_DIR</code> | Absolute Runtime-/Ausgabeverzeichnisse | Root leitet einen Harness-Elternpfad ab; direkter Harness erzeugt ein Work-Kind | Steuert ein getrenntes temporäres Layout. Root besitzt das private Runtime-/Harness-Material und <code>LOG_DIR</code>, einschließlich <code>NGINX_PHASE4_LOG_FILE</code> und des privaten <code>NGINX_MEMCHECK_EVIDENCE_DIR</code> unter <code>LOG_DIR/memcheck-evidence/&lt;case&gt;</code>; nur die getrennten <code>worker-state</code>- und <code>server-logs</code>-Bäume unter dem Worker-traversierbaren Harness-Root besitzen worker-eigene Leaves. Worker-Sichtbarkeit privater Ausgabe oder jede Überlappung schlägt fail-closed fehl. Vor jedem <code>mkdir</code>, Installieren, <code>chown</code>, <code>chmod</code>, <code>rm</code> oder jeder Output-Redirection müssen alle generischen/privaten Bootstrap-, Parent-Multi-Case- und Work-/Output-Roots pro Case sowie konfigurierbare Diagnostic-, Preflight-, Protocol-, Lifecycle- und Curl-Outputs als strikte Nachkommen von <code>VERIFIED_RUN_ROOT</code> validieren; ein Out-of-Root-Pfad endet mit <code>77</code>. <code>/dev/null</code> ist nur für die internen Bounded-Soak-Curl-Sinks erlaubt. Die separat geregelte Docroot-Projektionsausnahme wird in der nächsten Zeile beschrieben. Dieses Path-Authority-Control ändert die vertrauenswürdige <code>PYTHON</code>/<code>PATH</code>-Launch-Annahme nicht. |
| <code>NGINX_DOCROOT_PROJECTION</code>, <code>NGINX_DOCROOT_PROJECTION_PARENT</code>, <code>NGINX_DOCROOT_PROJECTION_ROOT</code> | Opt-in-Worker-sichtbare statische Docroot-Projektion; Boolean sowie absolute Parent- und exakte Kind-Pfade | <code>0</code>; Parent/Root leer, sofern sie nicht von einem vertrauenswürdigen Lifecycle-/Operator-Caller explizit übergeben werden | Der Projection-Modus schlägt fail-closed fehl, sofern der vertrauenswürdige Caller nicht sowohl einen externen Parent als auch ein exaktes frisches direktes Kind übergibt. Der Harness schlägt für diese Werte kein Lifecycle-Manifest nach und erzwingt keine Manifest-Registrierung; stattdessen validiert er, dass der Parent außerhalb privater Runtime-Roots bereits existiert, root-eigen, symlink-frei, für Gruppe/Andere nicht beschreibbar und nicht lesbar sowie in einer <code>0711</code>-sicheren Form für den Worker traversierbar ist; auch seine Ancestors müssen traversierbar sein. Der Helper erzeugt, <code>chown</code>t oder <code>chmod</code>t diesen Parent nie; er erzeugt nur das frische direkte Kind, kopiert die allowlisteten statischen Dateien und macht dieses Kind für den Worker traversierbar. Dies autorisiert keinen generischen Harness-Output außerhalb von <code>VERIFIED_RUN_ROOT</code>. |
| <code>NGINX_WORKER_USER</code>, <code>NGINX_WORKER_GROUP</code>, <code>NGINX_WORKER_PREFLIGHT_FILE</code>, <code>PERMISSIONS_LOG</code> | OS-Account/-Gruppe und absolute Log-Datei | <code>nobody</code>; Gruppe/Log abgeleitet | Wählt die tatsächliche NGINX-Worker-Identity und ihre Ownership-Grenze, nicht nur Diagnostik. Der direkte Harness erfordert Root-Ausführung und schlägt fail-closed fehl, sofern er keinen separaten verifizierbaren lokalen <code>NGINX_WORKER_USER</code> und keine Gruppe lösen kann; dann rendert er <code>user &lt;resolved-user&gt; &lt;resolved-group&gt;;</code>. Diese Identity besitzt nur Worker-State und die Server-Access-/Error-/Audit-Leaves. |
| <code>NGINX_MEMCHECK</code>, <code>NGINX_MEMCHECK_CASES</code>, <code>NGINX_MEMCHECK_DURATION_SECONDS</code>, <code>VALGRIND_BIN</code> | Opt-in-H1-Memcheck-Modus, begrenzte Case-Auswahl/Dauer und vertrauenswürdiges Valgrind-Executable | <code>NGINX_MEMCHECK=0</code>; <code>make memcheck-nginx</code> liefert seine begrenzten Defaults | Der Modus bleibt außerhalb von Default-Smoke/Test/CI. Er erfordert die Distinct-Worker-Prüfung bei Root-Ausführung und seine NGINX-Binary-/Archive-Identitätsgates. Der Summarizer vertraut nur effektive-UID-eigenen, nicht symbolisch verlinkten, nicht für Gruppe/Andere beschreibbaren Root-/Parent-Verzeichnissen; nur Direct-Child-private Single-Link-Regular-Evidence-Dateien, die mit No-Follow geöffnet und auf Ersetzung geprüft werden, können zu einem cleanen Ergebnis beitragen. Ein erhaltenes Pre-Hardening-Artifact ist kein Nachweis für diesen aktuellen Pfad. Der erhaltene Remediation-Beleg <code>$RUN/evidence/direct-nginx-h1-memcheck-evidence-remediation-20260801.md</code> (SHA-256 <code>37f01fe3d1851d43ae21d2b705b02bf01f204ff5cb19b41354e5b801a4b158a8</code>) zeichnet <code>passed_noncanonical_diagnostic</code> unter <code>umask 022</code> mit separatem <code>nobody</code>-Worker und privater Evidence-Metadata auf; er nutzt ein <code>pre-current-C</code>-Artifact. Ein separater frischer C-Build validierte vorherigen C-Code, während der finale Security-Scan und finale PR-Head-CI/Sonar-Evidence ausstehend bleiben. |
