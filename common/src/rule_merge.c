...
static int incomplete_remote(const char *key, const char *url) { return (key == 0) != (url == 0); }
...
int msconnector_rule_collection_merge(msconnector_rule_collection *out, const msconnector_rule_collection *parent, const msconnector_rule_collection *child) {
    ...
    if (child->rules_remote_key != 0 || child->rules_remote_url != 0) { if (incomplete_remote(child->rules_remote_key, child->rules_remote_url)) { return 0; } out->rules_remote_key = child->rules_remote_key; out->rules_remote_url = child->rules_remote_url; }
