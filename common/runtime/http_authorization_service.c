...
static int authorization_wait_for_shutdown(
    authorization_service *service,
    const msconnector_http_authorization_profile *profile,
    unsigned long timeout_ms) {
    ...
    if (authorization_defer_cleanup(service) != 0) {
         ...
         * uninterruptible runtime call. Profiles with a companion remain
         * quarantined: its shutdown callback is only valid after both
         * workers and the observer quiesce. */
...
static int serve_authorization(
    ...
    if (authorization_wait_for_shutdown(service, profile,
            cli->connection_timeout_ms) != 0) {
        return 1;
    }
