...
static int authorization_wait_for_workers(
    ...
    workers_finished = service->active_workers == 0UL;
...
static int serve_authorization(
    ...
    if (!authorization_wait_for_workers(service, cli->connection_timeout_ms)) {
        (void)fprintf(stderr,
            "%s worker shutdown grace period expired; forcing socket cancellation\n",
            profile->connector_name);
        authorization_shutdown_workers(service);
        if (!authorization_wait_for_workers(service, cli->connection_timeout_ms)) {
            const int deferred = authorization_defer_cleanup(service);
            (void)fprintf(stderr,
                "%s worker shutdown did not complete; runtime call may be uninterruptible\n",
                profile->connector_name);
            if (deferred != 0) {
                return 1;
            }
    ...
    if (!authorization_service_release(service)) {
        return 1;
    }
