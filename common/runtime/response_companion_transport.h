...
#define MSCONNECTOR_RESPONSE_COMPANION_TRANSPORT_MAX_HTTP_VERSION_SIZE 64U
/* Only a RESPONSE_HEADERS payload needs its fixed MRC1 field framing above
 * the Common logical header aggregate.  The generic cap remains in force for
 * every other opcode, including body, result, and control frames. */
#define MSCONNECTOR_RESPONSE_COMPANION_TRANSPORT_MAX_RESPONSE_HEADER_FRAME \
    (MSCONNECTOR_MAX_TOTAL_HEADER_BYTES + 2U + 2U + \
        MSCONNECTOR_RESPONSE_COMPANION_TRANSPORT_MAX_HTTP_VERSION_SIZE + 2U + \
        4U * MSCONNECTOR_MAX_HEADER_COUNT)
 ...
 */
/* Backend callback decisions borrow text pointers. The transport binds this
 * bounded state-owned scratch storage to each claimed session so an
 * owner-thread bridge can preserve native decision text until MRC1 has copied
 * it after the callback. P3/P4 callbacks receive the session itself as
 * read-only; they may write only through this pointer during the synchronous
