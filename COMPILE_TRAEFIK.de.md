...
Dieser Guide beschreibt den Einsatz des Traefik-Runtime-Pfads außerhalb dieses
Repositorys mit einem betreiberseitig bereitgestellten forwardAuth-
Decision-Service/Sidecar und externer Konfiguration.

## Status und Grenzen

Traefik wird von diesem Repository nicht aus Source gebaut. Externer Einsatz
bedeutet Traefik `forwardAuth` plus Decision-Service. Das Repository stellt
Beispielkonfiguration und Smoke-Pfade bereit, aber keine Produktionsreife.
...
1. Installieren oder stellen Sie das Traefik-Runtime-Binary über Ihre
...
2. Stellen Sie einen erreichbaren forwardAuth-Decision-Service/Sidecar bereit.
...
Dokumentieren Sie einen Traefik-Source-Build nicht als repository-unterstützt.
Source-basierter externer Einsatz bezieht sich auf libmodsecurity und/oder
betreiberseitig bereitgestellte Decision-Backend-Teile, während Traefik selbst
...
Siehe [examples/traefik/README.de.md](examples/traefik/README.de.md) für die
Erklärung dieser Direktiven, Platzhalter, Logs und Grenzen.

## Beispiel-Konfigurationen

Verwenden Sie die Dateien in [examples/traefik/](examples/traefik/README.de.md)
...
- Traefik wird von diesem Repository nicht aus Source gebaut.
- Traefik ist hier kein Production-Ready-Nachweis.
- Hier wird keine Go-Plugin-Implementierung bereitgestellt.
- RESPONSE_BODY / Phase 4 ist nicht promoted.
- Force-all-FAIL-Zeilen sind kein Produktionssupport.
- Decision-Service-Start ist betreiberseitig, sofern Ihr Deployment keinen
  eigenen hinzufügt.
