Sprache: [English](COMPILE_TRAEFIK.md) | Deutsch
...
Dieser Guide beschreibt den Einsatz des Traefik-Connectors oder Runtime-Pfads außerhalb dieses Repositorys. Er erklärt die drei Pfade von Voraussetzungen bis zum ersten Syntax-Check oder Start und verweist auf passende Beispiel-Konfigurationen.
...
2. Aus Paketen kommen typischerweise Runtime und Header/Bibliotheken. Das Artefakt `Traefik forwardAuth-Konfiguration` oder der Decision-Service muss weiterhin passend zu diesem Repository-Pfad gebaut, vorbereitet oder vom Betreiber bereitgestellt werden.
...
4. Bauen oder bereiten Sie den Pfad mit kompatiblen Paket-Headern/Bibliotheken vor. Benötigte Komponenten: Traefik-Binary plus vom Betreiber bereitzustellender Decision-Service.
...
3. Holen Sie dieses Repository und bauen/vorbereiten Sie `Traefik forwardAuth-Konfiguration` oder die vom Betreiber bereitzustellenden Backend-Teile.
4. Führen Sie den ersten Syntax-Check oder Start aus. Beispiele und Platzhalter stehen in [examples/traefik/README.de.md](examples/traefik/README.de.md).
...
Siehe [examples/traefik/README.de.md](examples/traefik/README.de.md) für die Erklärung dieser Direktiven, Platzhalter, Logs und Grenzen.

## Beispiel-Konfigurationen

Nutzen Sie die Dateien in [examples/traefik/README.de.md](examples/traefik/README.de.md) als Startpunkte für externe Konfiguration. Sie werden nicht automatisch vom Repository installiert und sind keine universellen Produktionsdefaults.
...
- Traefik wird von diesem Repository nicht aus Source kompiliert.
- Traefik ist hier nicht als production-ready belegt.
- Es gibt hier keine Go-Plugin-Implementierung.
- RESPONSE_BODY / Phase 4 ist nicht promoted.

## Verwandte Dokumente

- [English](COMPILE_TRAEFIK.md)
- [examples/traefik/README.de.md](examples/traefik/README.de.md)
