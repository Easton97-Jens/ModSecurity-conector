...
class HAProxySPOPTransactionCacheContractTests(unittest.TestCase):
    ...
    def test_missing_stateful_response_transaction_is_fail_closed(self) -> None:
        ...
        self.assertIn("set_response_correlation_failure(decision, phase)", response)
        ...
        self.assertIn('return "correlation-failure";', SOURCE)
        self.assertNotIn("set_missing_response_transaction_failure", response)
        self.assertNotIn('"deny", 503', response)
    ...
    def test_malformed_notify_fails_closed_in_both_runtime_modes(self) -> None:
        parser = SOURCE.split(
            "static int parse_notify_payload", 1
        )[1].split("static int build_notify_request_payload", 1)[0]
        handler = SOURCE.split(
            "static int handle_notify_frame", 1
        )[1].split("static int handle_connection", 1)[0]

        self.assertIn("len == 0U", parser)
        self.assertIn("return request->has_notify ? 0 : -1;", parser)
        self.assertIn("send_malformed_notify_outcome(fd, frame, state, log)", handler)
        self.assertIn("malformed NOTIFY rejected without ACK in legacy mode", handler)
        self.assertIn("return -1;", handler)
        self.assertNotIn("send_empty_ack(fd, frame, 2000U)", handler)

    def test_cache_miss_harness_uses_the_raw_spop_response_contract(self) -> None:
        self.assertIn('notify("check-response", "A")', HARNESS)
        self.assertIn('"stateful_response_transaction_missing_closed"', HARNESS)
