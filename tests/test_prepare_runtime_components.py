...
class PrepareRuntimeComponentsTest(unittest.TestCase):
    ...
    def test_component_failure_diagnostic_is_opt_in_target_bound_and_opaque(self) -> None:
        enabled = {
            components.RUNTIME_COMPONENT_FAILURE_DIAGNOSTICS_ENV: "1",
            "RUNTIME_COMPONENT_TARGET": "haproxy",
            components.HAPROXY_EVIDENCE_RECEIPT_ENV: "1",
        }
        malicious = "Authorization: Bearer hidden https://example.invalid/private body=must-not-appear"
        with mock.patch.object(components.sys, "stderr", new_callable=io.StringIO) as output:
            components.emit_runtime_component_failure_diagnostic(
                enabled,
                "expat",
                "expat-configure",
                2,
            ...
            components.emit_runtime_component_failure_diagnostic(
                {
                    components.RUNTIME_COMPONENT_FAILURE_DIAGNOSTICS_ENV: "enabled",
                    "RUNTIME_COMPONENT_TARGET": "haproxy",
                    components.HAPROXY_EVIDENCE_RECEIPT_ENV: "1",
                },
                "expat",
        ...
        self.assertEqual(
            ...
            [
                ...
                "component=expat build_step=expat-configure exit_code=2 "
                "classification=expat_build_failed",
                "prepare-runtime-components: temporary component failure diagnostic "
                "component=expat build_step=expat-configure exit_code=unavailable "
    ...
    def test_component_failure_diagnostic_reconciles_the_parsed_target_and_receipt(self) -> None:
        initial = {
            components.RUNTIME_COMPONENT_FAILURE_DIAGNOSTICS_ENV: "1",
            components.HAPROXY_EVIDENCE_RECEIPT_ENV: "1",
            "RUNTIME_COMPONENT_TARGET": "haproxy",
        }
        mismatched = dict(initial)
        components.configure_runtime_component_failure_diagnostics(mismatched, initial, "shared")

        self.assertEqual(mismatched["RUNTIME_COMPONENT_TARGET"], "shared")
        self.assertNotIn(components.RUNTIME_COMPONENT_FAILURE_DIAGNOSTICS_ENV, mismatched)
        with mock.patch.object(components.sys, "stderr", new_callable=io.StringIO) as output:
            components.emit_runtime_component_failure_diagnostic(
                mismatched,
                "expat",
        ...
        enabled = {}
        components.configure_runtime_component_failure_diagnostics(enabled, initial, "haproxy")
        self.assertEqual(enabled["RUNTIME_COMPONENT_TARGET"], "haproxy")
        self.assertEqual(enabled[components.RUNTIME_COMPONENT_FAILURE_DIAGNOSTICS_ENV], "1")
        self.assertEqual(enabled[components.HAPROXY_EVIDENCE_RECEIPT_ENV], "1")

    def test_expat_failure_diagnostic_preserves_failure_and_is_default_off(self) -> None:
        proc = subprocess.CompletedProcess(
            args=["configure"],
            returncode=2,
            stdout="cmake: command not found Authorization: Bearer hidden\\n",
            stderr="/private/source body=must-not-appear\\n",
        )
        cases = (
            (
                "enabled_haproxy",
        ...
        with tempfile.TemporaryDirectory(prefix="component-diagnostic-") as temporary:
            root = Path(temporary)
            for name, env, expected in cases:
                with self.subTest(name=name):
                    record: dict[str, object] = {}
                    with mock.patch.object(components, "run_env", return_value=proc) as run_env, mock.patch.object(
                        components.sys, "stderr", new_callable=io.StringIO
                    ) as output:
                        result = components.run_expat_build_step(
                            "expat-configure",
                            ["configure"],
                            cwd=root,
                            env=env,
                    ...
                    self.assertFalse(result)
                    self.assertEqual(record["status"], "failed")
                    self.assertEqual(record["blocker_reason"], "missing_cmake")
                    self.assertEqual(record["build_exit_code"], 2)
                    run_env.assert_called_once()
                    if expected:
                        self.assertIn("component=expat build_step=expat-configure exit_code=2", rendered)
    ...
    def test_modsecurity_failure_diagnostic_reports_only_the_first_closed_step(self) -> None:
        proc = subprocess.CompletedProcess(
            args=["sh", "./build.sh"],
            returncode=2,
        ...
        with tempfile.TemporaryDirectory(prefix="modsecurity-component-diagnostic-") as temporary:
            root = Path(temporary)
            staging_build = root / "staging-build"
            staging_build.mkdir()
            record: dict[str, object] = {}
            with mock.patch.object(components.shutil, "copytree"), mock.patch.object(
                components, "run_env", return_value=proc
            ) as run_env, mock.patch.object(components.sys, "stderr", new_callable=io.StringIO) as output:
                result = components.build_modsecurity_staging_entry(
                    env,
                    {"build_flags_text": "{}"},
                    {},
                    root / "source",
        ...
        self.assertEqual(record["build_exit_code"], 2)
        run_env.assert_called_once()
        self.assertEqual(run_env.call_args.args[0], ["sh", "./build.sh"])
        self.assertIn(
            "component=modsecurity build_step=modsecurity-build-sh exit_code=2 "
            "classification=modsecurity_build_failed",
            rendered,
        )
        for forbidden in (
            "hidden",
