...
class PrepareRuntimeComponentsTest(unittest.TestCase):
    ...
    def test_component_failure_diagnostic_is_opt_in_target_bound_and_opaque(self) -> None:
        ...
        self.assertEqual(
            ...
            [
                ...
                "classification=configure_c_compiler_unusable",
                "prepare-runtime-components: temporary component failure diagnostic "
    ...
    def test_component_failure_diagnostic_reconciles_parsed_target_and_receipt(self) -> None:
        initial = {
            components.RUNTIME_COMPONENT_FAILURE_DIAGNOSTICS_ENV: "1",
            components.HAPROXY_EVIDENCE_RECEIPT_ENV: "1",
            "RUNTIME_COMPONENT_TARGET": "haproxy",
    ...
    def test_expat_failure_diagnostic_preserves_record_and_maps_only_fixed_markers(self) -> None:
        proc = subprocess.CompletedProcess(
            args=["configure"],
            returncode=2,
            stdout=(
                "configure: error: No space left on device\n"
        ...
        cases = (
            (
                ...
                "classification=filesystem_full",
            ),
            (
                "default_off",
                {
        ...
        with tempfile.TemporaryDirectory(prefix="component-diagnostic-") as temporary:
            ...
            for name, env, expected in cases:
                with self.subTest(name=name):
                    ...
                    self.assertFalse(result)
                    self.assertEqual(record["status"], "failed")
                    self.assertEqual(record["blocker_reason"], "expat_build_failed")
    ...
    def test_modsecurity_failure_diagnostic_reports_only_first_closed_step(self) -> None:
        proc = subprocess.CompletedProcess(
            args=["sh", "./build.sh"],
            returncode=2,
