...
class CompilerGuideGenerationTest(unittest.TestCase):
    ...
    def test_connector_configuration_validation_and_runtime_stay_in_sections_seven_to_ten(self) -> None:
        ...
        envoy_prerequisites = numbered_section(guide("envoy"), 4)
        traefik_prerequisites = numbered_section(guide("traefik"), 4)
        self.assertIn("exact pinned Go 1.26 patch", envoy_prerequisites)
        self.assertIn("exact pinned Go 1.26 patch", traefik_prerequisites)
        self.assertIn('grep -Ex "go 1\\.26\\.[0-9]+" "$CONNECTOR_ROOT/connectors/envoy/ext_proc/go.mod"', envoy_prerequisites)
        self.assertIn(
            'grep -Ex "go 1\\.26\\.[0-9]+" "$CONNECTOR_ROOT/connectors/traefik/native_middleware/go.mod"',
            traefik_prerequisites,
        )
