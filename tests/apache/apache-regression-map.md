...
| `ModSecurity-apache/tests/regression/rule/00-basics.t` | non-matching `ARGS:test` request passes through | engine-core via Apache runtime | yes | query args, phase:2, pass-through | Does not assert debug log absence | `tests/common/cases/minimal/phase2_args_pass.yaml` |
| `ModSecurity-apache/tests/regression/action/10-logging.t` | `auditlog` action with deny/pass combinations | audit-log/engine-core via Apache runtime | partial | serial audit log, stable rule fields | Full action matrix and volatile audit fields stay out of common | `tests/common/cases/minimal/audit_log_phase1_block.yaml` |
