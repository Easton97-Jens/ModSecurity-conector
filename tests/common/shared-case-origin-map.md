...
| `v3-imported/v3_action_nolog_pass_no_audit.yaml` | `test/test-cases/regression/issue-2196.json` | ModSecurity_V3 | actions | `nolog,pass` rule does not create audit output | yes | fully-imported-common | actions, audit-log-absent, query args, phase1 | Uses `/` instead of upstream `index.php` for minimal fixture HTTP 200 |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `response_body_basic_block` | `tests/modsecurity-response-body.t`; `tests/regression/config/10-response-directives.t`; `test/test-cases/regression/variable-RESPONSE_BODY.json` | nginx/apache/v3 | response-body | partial | xfail | Dedicated probe kept outside active smoke: Apache returned HTTP 200 without audit hit; NGINX matched `RESPONSE_BODY` in phase 4 but the client observed `000`/empty reply, not HTTP 403 | response body, phase4, intervention, audit log | Not counted as common PASS and not executed by `smoke-common`; see `docs/response-body-blocking-investigation.md` |
