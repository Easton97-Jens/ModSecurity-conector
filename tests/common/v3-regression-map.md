...
| `test/test-cases/regression/issue-2196.json` | ModSecurity_V3 | v3/master observed 3.0.15 | actions | `nolog,pass` should not write audit output | yes | no | yes | `tests/common/cases/v3-imported/v3_action_nolog_pass_no_audit.yaml` | imported | actions, audit-log-absent, query-args, phase1 | Uses `/` instead of upstream `index.php` for minimal fixture HTTP 200 |
...
`BUILD_ROOT=/src/ModSecurity-conector-build`; Apache and NGINX both returned
...
| `v3_action_nolog_pass_no_audit.yaml` | `issue-2196.json` | fully-imported-common |
