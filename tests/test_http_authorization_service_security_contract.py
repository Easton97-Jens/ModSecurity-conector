...
class HttpAuthorizationServiceSecurityContractTests(unittest.TestCase):
    ...
    def test_listener_policy_allows_only_explicit_ipv4_bindings(self) -> None:
        ...
        self.assertIn('strcmp(host, "0.0.0.0") != 0', listen_parser)
        self.assertIn("return 0;", listen_parser)
    ...
    def test_common_content_length_parser_rejects_conflicting_duplicate_values(self) -> None:
        ...
        self.assertIn("if (seen && current != parsed) { return -1; }", parser)
        self.assertNotIn("if (seen) { return -1; }", parser)
