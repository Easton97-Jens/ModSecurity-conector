...
def runtime_selection_checks(source: str, helper: str, runtime: str, bodies: dict[str, str]) -> list[tuple[bool, str]]:
    ...
    return [
        ...
        ("HAPROXY_HTX_CANONICAL_RULES_FILE" in runtime and "phase1_403" in runtime and "phase1_429" in runtime and "phase3_403" in runtime and "enforced_reply" in runtime and "capability_promotion=not_permitted" in runtime, "runtime keeps real P1/P3 host replies explicit while withholding synthetic capability promotion"),
        ...
        ("phase4_safe_host_action=log_only" in runtime and "resolved_policy_action=log_only host_action=log_only" in runtime and "safe_log_only" in helper and "host_action = action == MSCONNECTOR_LATE_INTERVENTION_LOG_ONLY" in source, "runtime distinguishes the safe P4 log-only outcome from an unattempted Strict abort"),
...
def build_contract_checks(makefile_patch: str, build: str) -> list[tuple[bool, str]]:
    return [
        ("expected HAProxy 3.2.21" in build and "haproxy_modsecurity_htx_filter.c" in build and "haproxy-3.2.21-makefile.patch" in build, "build script is pinned to HAProxy 3.2.21 and stages a separate worktree"),
