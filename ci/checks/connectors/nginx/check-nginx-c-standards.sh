...
compile_profile() {
  ...
  case "$prof" in
    c17) std='-std=c17' ;;
    c23) std=$(python3 "$ROOT/ci/provisioning/toolchains/detect-c-standard.py" --profile c23 --compiler "$CC_BIN") || { rc=$?; [ "$rc" = 77 ] && { echo "SKIPPED: optional C23 check unsupported"; return 0; }; return "$rc"; } ;;
  ...
  for f in $files; do
    ...
    "$CC_BIN" $std -Wall -Wextra -Werror $incs -c "$ROOT/$f" -o "$obj"
  done
