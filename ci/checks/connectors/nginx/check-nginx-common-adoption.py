...
event_metadata_helper_end = common_h.index('\n\n#if !(NGX_PCRE)', event_metadata_helper_start)
...
('event.body.bytes_seen' not in access_event and 'event.body.bytes_seen' not in log_event and 'event.body.bytes_inspected' not in access_event and 'event.body.bytes_inspected' not in log_event and 'r->request_body' not in access_event and 'r->request_body' not in log_event, 'NGINX request event loggers keep helper output metadata-only and exclude request-body data'),
...
('if (r == NULL || mcf == NULL || mcf->phase4_log_file == NULL ||' in access_event and 'if (r == NULL || !msconnector_rule_id_validate(rule_id))' in log_event and 'ctx == NULL || mcf == NULL || mcf->phase4_log_file == NULL ||' in log_event and all('msconnector_event_write_jsonl_line' in event_block and 'ngx_write_fd' in event_block and 'mcf->phase4_log_file->fd' in event_block for event_block in (access_event, log_event)), 'NGINX request event loggers retain source-specific guards, rule-ID validation, and JSONL write paths'),
