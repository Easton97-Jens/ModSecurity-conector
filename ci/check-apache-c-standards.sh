...
compile_optional() {
    ...
    skip_message=$3
    set +e
    detected=$(python3 "$REPO_ROOT/ci/detect-c-standard.py" --profile "$profile" --compiler "$CC_BIN" 2>"$OUT/detect-$profile.err")
    rc=$?
    set -e
    if [ "$rc" -eq 77 ]; then
        echo "$skip_message"
...
case "$PROFILE" in
    ...
    all)
        ...
        ;;
    *)
        echo "usage: APACHE_C_STD_PROFILE={all|c17|c23|c2y} sh ci/check-apache-c-standards.sh" >&2
        exit 2
        ;;
esac
