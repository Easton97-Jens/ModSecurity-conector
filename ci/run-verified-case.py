...
def build_harness_command(
    ...
    if connector == "apache":
        ...
        harness = framework_root / "ci/run-apache-smoke.sh"
    elif connector == "nginx":
        env["NGINX_HARNESS_WORK_ROOT"] = str(paths["work"])
        harness = framework_root / "ci/run-nginx-smoke.sh"
    else:
        env["TMP_ROOT"] = str(paths["tmp"])
        env["LOG_ROOT"] = str(paths["work"] / "logs")
        env["RUNTIME_BASE"] = str(paths["runtime"])
        env["RUN_ONE_CASE"] = "1"
        harness = framework_root / "ci/run-haproxy-smoke.sh"

    cmd = ["sh", "ci/with-runtime-components.sh", "env"]
