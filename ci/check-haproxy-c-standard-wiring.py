...
if sh.exists():
 ...
 check('FRAMEWORK_COMMON' in txt and 'haproxy_include_flags_or_blocked' in txt, 'script uses framework common HAProxy header blocker helper')
 check('require_command_or_blocked' in txt, 'script uses framework common command blocker helper')
 check('modsecurity_include_flags_or_blocked' in txt, 'script uses framework common libmodsecurity header helper')
