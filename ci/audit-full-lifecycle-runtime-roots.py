    ...
    *,
    ...
    sources = {
        ...
        "BUILD_ROOT": {
            ...
            "source": "Makefile: VERIFIED_BUILD_ROOT defaults to VERIFIED_RUN_ROOT/build and ci/run-full-lifecycle-all-connectors.sh forwards it unchanged",
        },
        "EVIDENCE_ROOT": {
            "value": str(evidence_root),
            "source": "ci/run-full-lifecycle-all-connectors.sh defaults it from BUILD_ROOT/no-crs-evidence",
        },
        "NO_CRS_RAW_ROOT": {
            "value": str(build_root / "canonical-raw/no_crs_baseline"),
            "source": "ci/run-no-crs-baseline.sh default derived from BUILD_ROOT",
        },
        "CONNECTOR_COMPONENT_CACHE": {
            "value": str(component_cache_root),
            "source": "Makefile/ci/with-runtime-components.sh default derived from VERIFIED_RUN_ROOT/component-cache",
        },
...
def markdown(payload: dict[str, Any], language: str) -> str:
    ...
    german_findings = {
        ...
        "ci/run-no-crs-baseline.sh historically read runtime-env.sh and manifest.json from VERIFIED_RUN_ROOT/component-cache instead of CONNECTOR_COMPONENT_CACHE.": "Historisch las ci/run-no-crs-baseline.sh runtime-env.sh und manifest.json aus VERIFIED_RUN_ROOT/component-cache statt aus CONNECTOR_COMPONENT_CACHE.",
    ...
    for record in payload["targets"]:
        lines.extend(
            [
                ...
                f"- {labels['evidence']}: `{record['evidence_root']}`",
                f"- {labels['raw_run']}: `{record['run_root']}`",
                f"- {labels['build']}: `{record['build_root']}`",
                f"- {labels['cache']}: `{record['component_cache_root']}`",
                f"- {labels['expected']}: `{record['expected_binary']}`",
                f"- {labels['resolved']}: `{record['resolved_binary'] or labels['missing']}`",
