#!/bin/sh
# This file is sourced by the with-CRS Parent target after Framework common.sh.
# It deliberately replaces only the runtime snapshot's cache-owned CRS path
...
if [ "$verified_run_root_canonical" != "$VERIFIED_RUN_ROOT" ]; then
    ci_blocked "with-crs verified run root must not resolve through a symlink: $VERIFIED_RUN_ROOT"
    exit 77
fi

case "$fresh_crs_source_root" in
    "$CONNECTOR_COMPONENT_CACHE"|"$CONNECTOR_COMPONENT_CACHE"/*)
        ci_blocked "with-crs fresh CRS source root must not be inside CONNECTOR_COMPONENT_CACHE"
        exit 77
        ;;
esac
case "$CONNECTOR_COMPONENT_CACHE" in
    "$fresh_crs_source_root"|"$fresh_crs_source_root"/*)
        ci_blocked "with-crs CONNECTOR_COMPONENT_CACHE must not be inside the fresh CRS source root"
        exit 77
        ;;
esac

if [ -e "$fresh_crs_source_root" ] || [ -L "$fresh_crs_source_root" ]; then
    ci_blocked "with-crs fresh CRS source root must not exist before fetch: $fresh_crs_source_root"
    exit 77
fi
if [ -e "$fresh_crs_source_dir" ] || [ -L "$fresh_crs_source_dir" ]; then
    ci_blocked "with-crs fresh CRS source directory must not exist before fetch: $fresh_crs_source_dir"
    exit 77
fi
...
ci_info "with-crs using fresh CRS source root=$SOURCE_ROOT source=$CRS_SOURCE_DIR"
