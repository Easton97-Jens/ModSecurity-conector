...
validate_mrts_route() {
    ...
    case "${NO_CRS_RUN_ID:-}" in
        mrts-*) ;;
        *) echo "FAIL: no_crs_with_mrts requires a generated NO_CRS_RUN_ID" >&2; exit 77 ;;
    esac
    [ "${#NO_CRS_RUN_ID}" -eq 37 ] || {
        echo "FAIL: NO_CRS_RUN_ID has an invalid bounded length" >&2
        exit 77
    }
...
if [ "${MSCONNECTOR_MRTS_STAGE:-}" = no_crs_with_mrts ]; then
    ...
    if [ "$connector" != lighttpd ]; then
        ...
        # The patched Lighttpd route is native C.  Keep Go completely out of
        # its process environment, including cache and toolchain variables.
        unset GO GOTOOLCHAIN GOPATH GOMODCACHE GOCACHE GOTMPDIR GOENV
        unset MRTS_GO_BINARY MRTS_GO_BINARY_SHA256 MRTS_GO_VERSION
    fi
    ...
    MRTS_CLOSED_PLAN_SHA256=$MRTS_RUNTIME_PLAN_SHA256
...
reassert_mrts_closed_environment() {
    ...
    MRTS_RUNTIME_PLAN_SHA256=$MRTS_CLOSED_PLAN_SHA256
    ...
    NO_CRS_RUN_ID=$MRTS_CLOSED_NO_CRS_RUN_ID
    NO_CRS_BASELINE=1
    MODSECURITY_TEST_VARIANT=no-crs
    MODSECURITY_MRTS_VARIANT=with-mrts
    MSCONNECTOR_RULES_FILE=$MRTS_LOAD_FILE
    ...
    if [ "$connector" != lighttpd ]; then
        ...
        if [ "$PATH" != "$MRTS_CLOSED_PATH" ] || env | grep -Eq '^(GO|GOTOOLCHAIN|GOPATH|GOMODCACHE|GOCACHE|GOTMPDIR|GOENV|MRTS_GO_)='; then
            echo "FAIL: Lighttpd native runtime received Go toolchain state" >&2
            return 77
        fi
    fi
    ...
    require_mrts_python_invocation "$PYTHON_BIN" || {
    ...
    export CONNECTOR_ROOT FRAMEWORK_ROOT VERIFIED_RUN_ROOT BUILD_ROOT CACHE_ROOT TMP_ROOT LOG_ROOT RESULTS_DIR RUNTIME_ROOT RUNTIME_BASE RUNTIME_REPORT_OUTPUT_ROOT PATH PYTHON PYTHON_BIN HOME XDG_CACHE_HOME TMPDIR ALLOW_RUNTIME_DOWNLOADS ALLOW_RUNTIME_BUILDS MRTS_RUNTIME_PLAN MRTS_RUNTIME_PLAN_SHA256 MRTS_RUNTIME_RESULT MRTS_RUNTIME_EXECUTOR MRTS_RUNTIME_EXECUTOR_SHA256 MRTS_RUNTIME_RULES_ROOT MRTS_LOAD_FILE MRTS_CASE_ROOT MSCONNECTOR_MRTS_STAGE MSCONNECTOR_MRTS_RUNTIME NO_CRS_RUN_ID NO_CRS_BASELINE MODSECURITY_TEST_VARIANT MODSECURITY_MRTS_VARIANT MSCONNECTOR_RULES_FILE NO_CRS_RULES_FILE RULES_FILE MODSECURITY_RULE_PREAMBLE_FILE
}
