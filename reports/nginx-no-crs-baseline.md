...
|---|---|---|
| Response body availability (`response_body_buffered`) | `implemented_not_asserted` | A bounded NGINX body-filter path is wired; no current canonical real-host evidence proves that it processed a response body. |
...
| Rule evaluation (`phase4_rule_evaluation`) | `implemented_not_asserted` | The body-filter Phase-4 path is present; no current canonical real-host evidence proves that it evaluated rule `1100301`. |
| Pre-commit denial (`phase4_pre_commit_deny`) | `implemented_not_asserted` | A pre-header denial branch is present; no current canonical real-host evidence proves `requested_action=deny`, `actual_action=deny`, unsent headers, and visible `403`. |
