...
| lighttpd patched native | Gepatchte URI-clean-/Modul-Hooks | `msconnector_runtime_transaction_begin` pro Request | Reale 403 und 429 | Gepatchter decoded Body, P2 403 | Response-start, P3 403 | Deaktiviert: Hook sieht HTTP/1-Wire-Bytes | Request-EOS; Response wird beim Reset unbeobachtet geschlossen | Common liest verfügbare Entscheidungen | `http_status_set_err()` vor Commit | `handle_request_reset` beendet/zerstört genau einmal |
...
- lighttpd-P4 bleibt `NOT EXECUTED`: Der ausgewählte Output-Hook sieht Wire-Bytes. Ein Decoded-Entity-Hook mit Cursor-, Short-write/EAGAIN- und EOS-Semantik ist die nächste Voraussetzung.
