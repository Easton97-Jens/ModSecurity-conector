...
Am `2026-07-11` aus den eingecheckten Manifesten `connectors/<name>/capabilities.json` erzeugt. Es wurde kein kanonisches No-CRS-`result.json` verwendet, weil noch kein kanonischer Lauf ausgeführt wurde.
...
| Phase 4 | NOT IMPLEMENTED | Der ausgewählte SPOP-Pfad besitzt keine Response-Body-Route |
...
Der ausgewählte SPOP-Pfad besitzt ein begrenztes Request-Sample, aber keine
Response-Body-Route; Streaming-, Drop- und Abort-Entscheidungen sind nicht
implementiert. Separat existiert eine optionale HAProxy-3.2.21-HTX-Observer-
Quelle, die nicht ausgewählt, nur bodylos und keine kanonische Evidence ist.
...
Der ausgewählte SPOP-Antwortkörperpfad, `phase4` und
`phase4_rule_evaluation` sind `not_implemented`. Die optionale HTX-Observer-
Quelle ist in diesem Hostmodell nicht ausgewählt, umgeht bodytragende Requests
und ist kein kanonischer Phase-4-Laufzeitbeleg. Kein aktueller kanonischer
HAProxy-Lauf auf einem realen Host hat Regel `1100301` beobachtet. Die
semantischen Facetten für Vor-Commit-Deny, späte Aktionen und Statusmetadaten
sind `not_implemented`, weil der Runner kein beim Client sichtbares
Antwortergebnis, keinen tatsächlichen Commit-Zeitpunkt und keinen Hostpunkt
nach dem Commit beobachtet.

| Facette | Capability-Zustand | Erforderlicher Laufzeitbeleg vor einer Bestätigung |
|---|---|---|
| Antwortkörper-Verfügbarkeit (`response_body_buffered`) | `not_implemented` | Der ausgewählte SPOP-Pfad liefert keinen Response-Body; der optionale HTX-Observer ist nicht ausgewählt und nur bodylos. |
| Phase-4-Aufruf (`phase4`) | `not_implemented` | Kein ausgewählter SPOP-Response-Body-/EOS-Pfad existiert; der optionale HTX-Observer stuft diesen Zustand nicht hoch. |
| Regelauswertung (`phase4_rule_evaluation`) | `not_implemented` | Kein ausgewählter SPOP-Response-Body-Pfad existiert; kein kanonischer HAProxy-Hostlaufzeitbeleg belegt Regel `1100301`. |
