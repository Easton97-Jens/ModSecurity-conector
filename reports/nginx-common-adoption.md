...
| Request header iteration over `ngx_list_t` and request strings | `msconnector_request`, `msconnector_request_mapper_contract`, Common header helpers | kept as thin adapter | NGINX owns `ngx_list_t`; mapper emits Common request model, NUL-terminates request string fields, and validates it. |
| Response header iteration over `ngx_list_t` plus special headers | `msconnector_response`, `msconnector_response_mapper_contract` | kept as thin adapter | NGINX owns response state; mapper emits Common response model, synthesizes Content-Type/Content-Length metadata when needed, and validates it. |
...
| Phase-4 content-type file wildcards | NGINX exact-match-only validation after Common token validation | kept as thin adapter | NGINX runtime scope matching is exact, so wildcard entries are rejected during config load. |
