...
- Die gemeinsam genutzten minimalen YAML-Fälle unter `modules/ModSecurity-test-Framework/tests/cases/` definieren die
  rule/request/expectation Modell, das von Apache und NGINX verwendet wird.
- `modules/ModSecurity-test-Framework/tests/runners/case_cli.py` liest jede YAML-Datei und materialisiert den Apache
...
Die dem Adapter gehörende Quelle entstand aus `connectors/apache/src` während der Beibehaltung
der beobachtete Autotools/APXS Pfad:
...
Offene Apache PoC-Follow-ups werden nachverfolgt
