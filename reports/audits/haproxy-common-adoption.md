...
`ci/checks/connectors/haproxy/check-haproxy-c-standards.sh` performs a hard C17 object compile when HAProxy and libmodsecurity headers are discoverable. C23 and future-C (`c2y`) are optional and report skipped when unsupported by the compiler. Missing HAProxy or libmodsecurity headers are reported as `BLOCKED` with exit code 77.
