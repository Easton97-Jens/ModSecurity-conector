...
`modules/ModSecurity-test-Framework/ci/checks/catalog/no_crs_baseline.py`, the no-CRS
...
from `ci/provisioning/components/prepare-runtime-components.py`, the framework's
`modules/ModSecurity-test-Framework/ci/provisioning/prepare-nginx-build.sh`, and
...
`ci/checks/protocol/protocol_client.py` always writes these payload-free artifacts for a
managed probe:
