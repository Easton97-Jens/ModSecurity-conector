...
- `sh -n ci/runtime/lifecycle/run-connector-stage.sh` and
  `shellcheck --severity=error ci/runtime/lifecycle/run-connector-stage.sh`
  passed.
- `/root/git/ModSecurity-conector/.venv/bin/python -m pip check` passed with
  no broken requirements.
- The new hermetic missing-case regression and the existing target-separation
  control each passed through the selected Parent virtual environment.
- `tests.test_bilingual_docs` passed 22 tests.
- The complete `tests.test_no_crs_selected_runner_wiring` module ran seven
  tests and exited 1 only for two Apache fixture subcases below the
  intentionally uninitialized Parent-recorded Framework Gitlink. This is a
  `blocked_external_dependency`, not a passed module check; neither Framework
  contents nor its Gitlink were changed to bypass it.
- The staged merge result passed `git diff --check` and contains only the six
  scoped files listed above.

The current exact PR head, Security-Diff review, hosted checks, review state,
SonarQube Cloud analysis, merge result, and post-merge master evidence remain
required and are not claimed by this record.
