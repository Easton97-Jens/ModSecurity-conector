...
- Ein Response-seitiger SPOP-Cache-Korrelationsfehler ist getrennt fail-closed, auch wenn ein Betreiber gewöhnliche Enginefehler mit `fail-mode=open` gewählt hat: begrenzte Eviction kann ein Verfügbarkeitslimit bleiben, aber ein späteres Response-NOTIFY liefert jetzt `deny`/`503`, `disruptive=1` und `stateful_response_transaction_missing_closed`, statt Response-Enforcement still zu überspringen.
...
- Apache-Contracts: 4 bestanden; das aktuelle Source-Modul baute gegen Apache 2.4.66/APXS und isolierte echte Hostfälle lieferten Allow `200`, P1-Block `403` und P2-Block `403`; task-eigene Ports und Testprozesse waren nach jedem Fall entfernt. Dies ist keine Same-Host-Folge-Evidence nach einem Fehler. Die optionalen synchronisierten `engine-limit`- und `client-abort`-Fixtures waren vor der Upstream-Adressveröffentlichung blockiert.
- NGINX-Contracts: 13 bestanden, einschließlich der fail-closed-Regression Zero-Return zu 500. Die aktuelle Source baute gegen gepinntes NGINX 1.31.4 und ein nativer Host lieferte Allow `200`, Header-Block `403`, danach Allow `200`; Port `29183` wurde freigegeben. Der kanonische Sandbox-Harness war durch `chown(..., nobody)` `EINVAL` blockiert; ein task-eigener Listener aus diesem Fehlversuch wurde separat ownership-verifiziert und kontrolliert bereinigt.
- Direkte C-API-Contracts: 3 bestanden.
- HAProxy-SPOP-Source-Contracts: 8 fokussierte Prüfungen bestanden; die aktuelle Produktionsagent-Kontrolle `run_haproxy_spop_cache_miss.sh` mit `max-transactions=1` beobachtete Cache-Miss `deny`/`503`/`stateful_response_transaction_missing_closed`, echten Rule-Block `403`, frisches Allow `200` und Agent-Cleanup. GCC-/Clang-Runtime-Self-Tests, Reset/EPIPE, Peer-Isolation, sofortiges Schließen eines gesättigten Peers, HELLO-Deadline und Folgekontrollen bestanden ebenfalls.
...
Die aufbewahrte Evidence deckt Engine-Startverfügbarkeit, Fehler innerhalb einer Transaktion, Timeout, ungültige/unvollständige Peer-Eingaben, Client-/Peer-Reset, unvollständige HELLO- und Body-/Stream-Grenzen, soweit der lokale Harness sie bereitstellt, parallele Requests/Streams, Größen-/Admission-Grenzen, Cancel, Shutdown, Folgekontrollen und Cleanup ab. SPOP-Logs erfassen Reset `errno=104`, EPIPE `errno=32` und den aktuellen Cache-Miss `503`, gefolgt von echten Block- und Allow-Kontrollen. Aktuelle Apache- und NGINX-Hosts halten ebenso Allow-/Block-/Folgebeobachtungen und Listener-Cleanup fest. Der Native-UDS-Test prüft nach Reset und 64-Peer-Write-Deadline-Erschöpfung eine legitime Folgeanfrage sowie das Fehlen des zugewiesenen UDS-Listeners.

## Nicht ausgeführte Prüfungen mit Begründung

- Kein nativer HAProxy-HTX-Host war verfügbar. NGINX und Apache haben nur die oben beschriebenen begrenzten aktuellen Hostkontrollen, keine vollständige Hostvektor-Abdeckung; der kanonische NGINX-Harness bleibt sandbox-blockiert.
