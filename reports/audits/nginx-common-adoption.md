...
| Host fallback from `r->headers_in.server` | `msconnector_headers_find_first` plus NGINX-specific fallback | kept as thin adapter | NGINX maps Host through Common header lookup and then falls back to NGINX request fields; `msconnector_headers_host` is not claimed as adopted by NGINX in this PR. |
...
| C standard checks | `ci/checks/connectors/nginx/check-nginx-c-standards.sh` | replaced | C17 is hard when headers exist; C23/future-C are optional and skip safely. |
...
| Phase-4 body limit directive | `MSCONNECTOR_DIRECTIVE_PHASE4_BODY_LIMIT` and `msconnector_parse_size` | replaced | NGINX registers the Common-known directive, maps it into `common_config.phase4_body_limit`, counts all seen bytes separately from inspected bytes, and stops passing bytes to ModSecurity after the configured limit while setting truncation metadata. |
| Apache-style transaction ID expression directive | `MSCONNECTOR_DIRECTIVE_TRANSACTION_ID_EXPR` | kept as not applicable | NGINX does not register Apache expression syntax; `modsecurity_transaction_id` remains the NGINX complex-value directive. |
