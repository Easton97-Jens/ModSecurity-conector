...
> gelten der generierte [No-CRS-Gesamtsnapshot](../current/all-connectors-no-crs-baseline.de.md)
...
`modules/ModSecurity-test-Framework/ci/lib/common.sh` ist die Quelle der Wahrheit für
...
`modules/ModSecurity-test-Framework/ci/lib/connector-smoke-common.sh` und Suchen
...
`<verified-run-root>` oder `<verified-run-root>`
...
Artefakt ist ein kopiertes Nachweisbündel aus `<verified-run-root>/`
