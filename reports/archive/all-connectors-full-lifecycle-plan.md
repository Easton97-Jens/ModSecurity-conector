...
- `ci/runtime/common/resolve-runtime-paths.py` centralizes validated per-connector evidence,
...
- Framework: `modules/ModSecurity-test-Framework/tests/cases/no-crs-baseline/`
  and `ci/checks/catalog/no_crs_baseline.py`.
...
`ci/provisioning/components/prepare-runtime-components.py` uses cache schema v2 and keys reusable
