...
|---|---|---|
| Antwortkörper-Verfügbarkeit (`response_body_buffered`) | `implemented_not_asserted` | Ein begrenzter Apache-Ausgabefilterpfad ist verdrahtet; kein aktueller kanonischer Laufzeitbeleg auf einem realen Host belegt die Verarbeitung eines Antwortkörpers. |
...
| Deny vor dem Commit (`phase4_pre_commit_deny`) | `not_implemented` | Die Antwortkörperentscheidung wird bei EOS nach dem Header-Pfad finalisiert; der Host besitzt keinen deterministischen, noch nicht festgeschriebenen Body-Entscheidungspunkt und behauptet kein sichtbares Phase-4-`403`. |
